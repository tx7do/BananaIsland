...
//		GameObject.Find("TextFreeCoinsDown").renderer.sortingLayerID = 1;
		...
		if(Input.GetMouseButtonUp(0))// || Input.GetKeyDown(KeyCode.Space))
		{
			...
			else if(releasedItem == "PauseHolePlay") // KLIKNUTO NA PLAY DUGME IZ PAUSE MENIJA
			{
				...
				if(playerStopiran)
				{
					...
					GameObject.Find("ButtonPause").collider.enabled = true;
			...
			else if(releasedItem == "FailedMainHole") // KLIKNUTO NA MAIN DUGME IZ FAILED MENIJA
			{
				...
				temp.animation.Play("FinishButtonsClick");
				...
				StartCoroutine(FailedScreenHolder.transform.GetChild(0).animation.Play("FailedGo", false, what => {helpBool=true;}) );
			...
			else if(releasedItem == "ButtonRestart1") // KLIKNUTO NA RESTART DUGME IZ FINISH MENIJA
			{
				...
				temp.animation.Play("FinishButtonsClick");
				command = delegate {Application.LoadLevel(Application.loadedLevel);};
				StartCoroutine(Win_CompletedScreenHolder.transform.GetChild(0).animation.Play("FinishTableGo1", false, what => {helpBool=true;}) );
				StartCoroutine(DoAfterAnimation(temp,"FinishButtonsClick"));
			...
			{
				GameObject.Find(releasedItem).animation.Play("FinishButtonsClick");
			...
			{
				GameObject.Find(releasedItem).transform.GetChild(0).animation.Play("FinishButtonsClick");
				shopHolder.transform.position = Camera.main.transform.position + Vector3.forward*5;
				StartCoroutine(OpenFreeCoinsCard());
			}
			...
			else if(releasedItem == "PowersCardCoinx2")
			{
				...
				powerCard_CoinX2.collider.enabled = false;
				brojDoubleCoins--;
				powerCard_CoinX2.GetChild(3).GetChild(0).GetComponent<TextMesh>().text = brojDoubleCoins.ToString();
				powerCard_CoinX2.GetChild(3).GetChild(1).GetComponent<TextMesh>().text = brojDoubleCoins.ToString();
			...
			{
				//GameObject temp = GameObject.Find(releasedItem);
				powerCard_Magnet.collider.enabled = false;
				brojMagneta--;
				powerCard_Magnet.GetChild(3).GetChild(0).GetComponent<TextMesh>().text = brojMagneta.ToString();
				powerCard_Magnet.GetChild(3).GetChild(1).GetComponent<TextMesh>().text = brojMagneta.ToString();
				kupljenMagnet = true;
			...
			{
				//GameObject temp = GameObject.Find(releasedItem);
				powerCard_Shield.collider.enabled = false;
				brojShieldova--;
				powerCard_Shield.GetChild(3).GetChild(0).GetComponent<TextMesh>().text = brojShieldova.ToString();
				powerCard_Shield.GetChild(3).GetChild(1).GetComponent<TextMesh>().text = brojShieldova.ToString();
				kupljenShield = true;
	...
	{
		while(obj.animation.IsPlaying(animationName))
			yield return null;
		command();
	...
	{
		StartCoroutine(pauseButton.transform.GetChild(0).animation.Play("FinishButtonsClick", false, what => {helpBool=true;}) );
		StartCoroutine( pauseScreenHolder.transform.GetChild(0).animation.Play("PauseShow", false, what => {helpBool=true;}) );
		while(!helpBool)
		{
			yield return null;
		}
	...
	{
		StartCoroutine(pauseButton.transform.GetChild(0).animation.Play("FinishButtonsClick", false, what => {helpBool=true;}) );
		StartCoroutine( pauseScreenHolder.transform.GetChild(0).animation.Play("PauseGo", false, what => {helpBool=true;}) );
		while(!helpBool)
		{
			yield return null;
		}
	...
	{
		StartCoroutine( GameObject.Find("ButtonMain_Pause").animation.Play("FinishButtonsClick", false, what => {helpBool=true;}) );
		StartCoroutine( pauseScreenHolder.transform.GetChild(0).animation.Play("PauseGo", false, what => {helpBool=true;}) );
		while(!helpBool)
		{
			yield return null;
	...
	{
		StartCoroutine( GameObject.Find("ButtonRestart_Pause").animation.Play("FinishButtonsClick", false, what => {helpBool=true;}) );
		StartCoroutine( pauseScreenHolder.transform.GetChild(0).animation.Play("PauseGo", false, what => {helpBool=true;}) );
		while(!helpBool)
		{
			yield return null;
	...
	IEnumerator unPause()
	{
		StartCoroutine( GameObject.Find("ButtonPlay_Pause").animation.Play("FinishButtonsClick", false, what => {helpBool=true;}) );
		StartCoroutine(dropPauseScreen());
		while(!helpBool)
		{
			yield return null;
	...
	void showFailedScreen()
	{
		...
		FailedScreenHolder.transform.GetChild(0).animation.Play("FailedShow");
	}
	...
	void ShowWinScreen()
	{
		...
		Win_CompletedScreenHolder.transform.GetChild(0).animation.Play("FinishTableShow");
	...
	IEnumerator CheckKeys()
	{
		if(keysCollected == 1)
		{
			...
			yield return new WaitForSeconds(0.75f);
			keyHole2.animation.Play("FinishKeyNo");
			yield return new WaitForSeconds(0.25f);
			keyHole3.animation.Play("FinishKeyNo");
		}
		...
		else if(keysCollected == 3)
		{
			...
			yield return new WaitForSeconds(0.5f);
			Win_CompletedScreenHolder.transform.GetChild(0).animation.Play("FinishPriceClick");
		...
		{
			yield return new WaitForSeconds(0.75f);
			keyHole1.animation.Play("FinishKeyNo");
			yield return new WaitForSeconds(0.25f);
			keyHole2.animation.Play("FinishKeyNo");
			yield return new WaitForSeconds(0.25f);
			keyHole3.animation.Play("FinishKeyNo");
	...
	IEnumerator waitForStars()
	{
		...
		star1.animation.Play("FinishStars1");
		star1.transform.GetChild(0).gameObject.SetActive(true);
		yield return new WaitForSeconds(0.5f);
		GameObject.Find("FinishButtonTable").animation.Play("FinishShakingTableStars");
		//star1.transform.GetChild(0).gameObject.SetActive(true);
		starsGained = 1;
		...
		if(coinsCollected >= 70)
		{
			...
			star2.animation.Play("FinishStars2");
		...
		if(coinsCollected >= 90)
		{
			...
			star3.animation.Play("FinishStars3");
	...
	void KeyCollected()
	{
		keysCollected++;
		if(keysCollected == 1)
			GameObject.Find("GamePlayKeyHole1").animation.Play();
		else if(keysCollected == 2)
			GameObject.Find("GamePlayKeyHole2").animation.Play();
		else if(keysCollected == 3)
			GameObject.Find("GamePlayKeyHole3").animation.Play();
	}
...
//		}
		if(progressBarScale.localScale.y >= 1)
		{
			...
			progressBarScale.GetChild(0).GetChild(0).renderer.material.mainTextureScale = new Vector2(1,1);
		}
	}
	...
	IEnumerator graduallyFillTile(float value)
	{
		...
		float result = progressBarScale.GetChild(0).GetChild(0).renderer.material.mainTextureScale.y + value;
		float t = 0;
		while(t < result)
		{
			yield return null;
			if(progressBarScale.localScale.y < 1)
			progressBarScale.GetChild(0).GetChild(0).renderer.material.mainTextureScale = Vector2.Lerp(progressBarScale.GetChild(0).GetChild(0).renderer.material.mainTextureScale, new Vector2(1, result), 0.2f);
			t += Time.deltaTime*2;
		}
	}
	...
	IEnumerator BuyKeys()
	{
		buttonBuyKeys.collider.enabled = false;
		if(keysCollected == 0)
		{
