...
public class LanguageManager : MonoBehaviour
{
    ...
    public IEnumerator helpfunk(TextMesh toAdjust)
    {
        ...
        while (((toAdjust.GetComponent<Renderer>().bounds.size.x * 0.9f) >
                toAdjust.GetComponent<Collider>().bounds.size.x) &&
               ((toAdjust.GetComponent<Renderer>().bounds.size.y * 0.9f) >
                toAdjust.GetComponent<Collider>().bounds.size.y))
        {
        ...
        do
        {
            ...
            while ((counter < wholeText.Length))
            {
                ...
                while (((toAdjust.GetComponent<Renderer>().bounds.size.x <
                         toAdjust.GetComponent<Collider>().bounds.size.x) || toAdjust.text ==
                           wholeText[((counter < wholeText.Length) ? counter : wholeText.Length - 1)]) &&
                       counter < wholeText.Length)
                {
                    previous = toAdjust.text;
                    toAdjust.text += ((chosenLanguage == "_jp" || chosenLanguage == "_ch") ? "" : " ") +
                                     wholeText[counter];
                    counter++;
                    helpCounter++;
                    yield return null;
            ...
            charSize *= 0.9f;
            Debug.Log("enddd");
        } while (toAdjust.GetComponent<Renderer>().bounds.size.y > toAdjust.GetComponent<Collider>().bounds.size.y ||
                 again);
    }
            ...
            do
            {
                ...
                Debug.Log("enddd");
                      ...
                      (toAdjust.GetComponent<Renderer>().bounds.size.x >
                       toAdjust.GetComponent<Collider>().bounds.size.x)) || again);

            Debug.Log(
                (toAdjust.GetComponent<Renderer>().bounds.size.y > toAdjust.GetComponent<Collider>().bounds.size.y) +
                "###" +
                (toAdjust.GetComponent<Renderer>().bounds.size.x > toAdjust.GetComponent<Collider>().bounds.size.x) +
                "###" + again);
        }
