    ...
    void Update()
    {
        if (canScroll)
        {
            ...
            else if (Input.GetMouseButton(0))
            {
                ...
                offsetY = endY - startY;
                items.position = new Vector3(items.position.x, /*Mathf.Clamp(*/
                    Mathf.MoveTowards(items.position.y, items.position.y + offsetY, 0.5f) /*,downLimitY,upLimitY)*/,
                    items.position.z);
                startY = endY;
            }
            ...
            if (released && startY == endY)
            {
                ...
                {
                    //offsetY = -offsetY;
                    items.position = new Vector3(items.position.x, Mathf.MoveTowards(items.position.y, downLimitY, 1f),
                        items.position.z);
                }
                else if (items.position.y > upLimitY)
                {
                    //offsetY = -offsetY;
                    items.position = new Vector3(items.position.x, Mathf.MoveTowards(items.position.y, upLimitY, 1f),
                        items.position.z);
                }
