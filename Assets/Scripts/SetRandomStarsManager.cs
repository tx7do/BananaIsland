...
public class SetRandomStarsManager : MonoBehaviour
{
    ...
    public void GoBack()
    {
        ...
        else
        {
            ...
            {
                string pom = System.String.Empty;
                StagesParser.allLevels[currSet * 20 + currStage] = (currSet * 20 + currStage + 1).ToString() + "#" +
                                                                   starsGained + "#" + Manage.points;
                ...
                if (StagesParser.currSetIndex != 5 ||
                    StagesParser.currStageIndex != 19) //bilo je StagesParser.currSetIndex != 4
                {
                    ...
                    if (currStage < 19 && int.Parse(values[1]) == -1)
                    {
                        ...
                        StagesParser.allLevels[currSet * 20 + currStage + 1] =
                            (currSet * 20 + currStage + 2).ToString() + "#0#0";
                        for (int i = 0; i < StagesParser.allLevels.Length; i++)
                        {
                            pom += StagesParser.allLevels[i];
                            pom += "_";
            ...
            PlayerPrefs.SetInt("TrenutniNivoNaOstrvu" + (StagesParser.currSetIndex).ToString(),
                StagesParser.trenutniNivoNaOstrvu[StagesParser.currSetIndex]);
            PlayerPrefs.Save();
            ...
            PlayerPrefs.Save();
            Debug.Log("potrebne zvezdice: " + potrebneZvezdice + ", a ima gi: " + StagesParser.currentStarsNEW +
                      ", last unlocked world index: " + StagesParser.lastUnlockedWorldIndex);
            Debug.Log("Unlocked worlds: " + StagesParser.unlockedWorlds[0] + ", " + StagesParser.unlockedWorlds[1] +
                      ", " + StagesParser.unlockedWorlds[2] + ", " + StagesParser.unlockedWorlds[3] + ", " +
                      StagesParser.unlockedWorlds[4] + ", " + StagesParser.unlockedWorlds[5]);
            ...
            if (uslovNivo && uslovZvezdice)
            {
                ...
                StagesParser.allLevels[StagesParser.lastUnlockedWorldIndex * 20] =
                    (StagesParser.lastUnlockedWorldIndex * 20 + 1) + "#0#0";
                ...
                if (StagesParser.lastUnlockedWorldIndex == 5 && FB.IsLoggedIn) //@@@@@@ DODATAK ZA NOVA OSTRVA
                {
                    for (int i = 0; i < FacebookManager.ListaStructPrijatelja.Count; i++)
                    {
                        if (FacebookManager.ListaStructPrijatelja[i].PrijateljID.Equals(FacebookManager.User))
                        {
                            ...
                            {
                                for (int j = FacebookManager.ListaStructPrijatelja[i].scores.Count;
                ...
                PlayerPrefs.SetString("AllLevels", pom);
                PlayerPrefs.Save();
                Debug.Log("StagesParser.allLevels[StagesParser.lastUnlockedWorldIndex*20] = " +
                          StagesParser.allLevels[StagesParser.lastUnlockedWorldIndex * 20] + ", treba upisat': " +
                          ((StagesParser.lastUnlockedWorldIndex * 20 + 1) + "#0#0"));
