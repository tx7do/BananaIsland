...
//						}
				...
				else if(tip == Tip.Bunika)
				{
					...
					anim.Play("Blong");
					playerController.rigidbody2D.velocity = Vector2.zero;
					playerController.rigidbody2D.AddForce(new Vector2(1000,1000));
					...
					GameObject.Find("Background").renderer.material.color = new Color(0.82f,0.07f,0.75f,1);
				}
...
//		Instantiate(GameObject.Find("PrinceGorilla"),playerController.transform.position,Quaternion.identity);
		...
		goal.animation.Play("MonkeyBounceFromMushroom");
		...
		playerController.rigidbody2D.drag = 0;
		playerController.animator.Play(playerController.fall_State);
		while(goal.animation.IsPlaying("MonkeyBounceFromMushroom"))
		{
