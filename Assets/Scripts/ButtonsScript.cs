...
public class ButtonsScript : MonoBehaviour {
	...
	void AnimateButton(GameObject btn,int Onoff)
	{
		if(btn.name!="PlayerName")
		{
			if(Onoff== 1)
			{
				...
				save= new Material(btn.renderer.sharedMaterial);
				Material nov =new Material(btn.renderer.sharedMaterial);
				nov.color=new Color(save.color.r-0.2f,save.color.g-0.2f,save.color.b-0.2f,save.color.a);
				btn.renderer.sharedMaterial=nov;
			}
			else
			{	
				btn.transform.localScale= scaleSave;
				btn.renderer.sharedMaterial=save;
			}
		}
	}
	...
	{
		Ray rej=GameObject.Find("Main Camera").camera.ScreenPointToRay(v+Vector3.forward*10);
		RaycastHit hit;
