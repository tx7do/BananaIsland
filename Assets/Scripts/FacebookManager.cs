...
//				MainScene.LeaderBoardInvite.SetActive(false);
    ...
    {
        MainScene.FacebookLogIn
            .SetActive(false); //Ovde izbacio MissingReferenceException kad sam se vratio na main scenu i odmah isao na play
...
//			Application.OpenURL("https://www.facebook.com/pages/Banana-Island/636650059721490");
            ...
            if (otisaoDaLajkuje)
            {
                ...
                if (stranica == "BananaIsland")
                {
                    ...
                    ShopManagerFull.ShopObject.transform
                        ...
                        .color = new Color(0.58f, 0.58f, 0.58f);
                    StagesParser.currentMoney += StagesParser.likePageReward;
                    PlayerPrefs.SetInt("TotalMoney", StagesParser.currentMoney);
                    PlayerPrefs.Save();
                    StartCoroutine(StagesParser.Instance.moneyCounter(StagesParser.likePageReward,
            ...
            if (PlayerPrefs.HasKey("Logovan"))
            {
                if (PlayerPrefs.GetInt("Logovan") == 0)
                {
                    if (FB.IsLoggedIn)
                    {
                        ...
                        if (MestoPozivanjaLogina == 1)
                        {
                            Transform leaderboardTabs =
                                MainScene.LeaderBoardInvite.transform.parent.Find("Friends Tabs");
                            Transform[] FriendsObjects = new Transform[leaderboardTabs.childCount];
                            for (int i = 0; i < leaderboardTabs.childCount; i++)
                            {
                                leaderboardTabs.GetChild(i).Find("Friend/LeaderboardYou").GetComponent<SpriteRenderer>()
                                    .enabled = false;
                                FriendsObjects[i] = leaderboardTabs.GetChild(i);
                            }

                            Transform leaderboardLogin =
                                MainScene.LeaderBoardInvite.transform.parent.Find("Friends Tabs/Friend No 2");
                            leaderboardLogin.localPosition = new Vector3(leaderboardLogin.localPosition.x, -1.85f,
                                leaderboardLogin.localPosition.z);
                            MainScene.FacebookLogIn.SetActive(true);
                            MainScene.LeaderBoardInvite.SetActive(false);
                            for (int i = 0; i < 10; i++)
                            {
    ...
    {
        FB.API("/" + User + "/picture?redirect=true&height=64&type=normal&width=64", Facebook.HttpMethod.GET,
            MyPictureCallback);
    }
    ...
    public void GetFacebookNameCallback(FBResult result)
    {
        ...
        {
            Dictionary<string, object> rez =
                Facebook.MiniJSON.Json.Deserialize(result.Text) as Dictionary<string, object>;
...
//			              );
    ...
    void InviteCallback(FBResult result)
    {
        ...
        {
            GameObject.Find("INTERFACE HOLDER/_TopLeft/Coins/Coins Number").GetComponent<TextMesh>().text =
                StagesParser.currentMoney.ToString();
            GameObject.Find("INTERFACE HOLDER/_TopLeft/Coins/Coins Number").GetComponent<TextMeshEffects>()
...
//	 string FeedLinkDugacakOpis = "Trenutni ostrvo je ";
    string LinkSlike =
        "https://trello-attachments.s3.amazonaws.com/52fb9e010aaea80557f83f1f/1024x500/d64a4cc319932dde0630258aee32d7d4/Feature-Graphic.jpg"; //preporucuje se slika 600x600, minimum je 200x200
    ...
    public void
        CallFBFeed(string ImeOstrva,
            int BrojNivoa) //bug za iOS u xcode projektu ispravka http://stackoverflow.com/questions/21022557/unity-fb-sdk-on-ios-problems-with-fb-feed-function
    {
...
//		FB.API("1474610946102580?fields=scores",Facebook.HttpMethod.GET,GetScoreCallback);  // vraca sve HighScorove
    ...
    public void GetFacebookFriendScores()
    {
        ...
        FB.API("1609658469261083/scores", Facebook.HttpMethod.GET,
            GetFacebookFriendScoresCallback); // vraca sve HighScorove Prijatelja 
    }
        ...
        else
        {
            ...
            if (MestoPozivanjaLogina == 1)
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    Camera.main.GetComponent<MainScene>().friendsObjects[i].Find("FB Invite").gameObject
                        .SetActive(false);
                    Camera.main.GetComponent<MainScene>().friendsObjects[i].Find("Friend").gameObject
                        .SetActive(true);
                }
                ...
                for (int j = 0; j < Scorovi.Count; j++)
                {
                    ...
                    if (j < 10)
                    {
                        ...
                        GameObject.Find("Pts Number " + broj.ToString()).GetComponent<TextMeshEffects>()
                            .RefreshTextOutline(true, true);

                        GameObject.Find("Name " + broj.ToString()).GetComponent<TextMesh>().text = Imena[j];
                        GameObject.Find("Name " + broj.ToString()).GetComponent<TextMeshEffects>()
                            .RefreshTextOutline(true, true);

                        if (Korisnici[j] == FB.UserId)
                        {
                            Camera.main.GetComponent<MainScene>().friendsObjects[j].Find("Friend/LeaderboardYou")
                ...
                {
                    Camera.main.GetComponent<MainScene>().friendsObjects[j].Find("FB Invite").gameObject
                        .SetActive(true);
                    Camera.main.GetComponent<MainScene>().friendsObjects[j].Find("FB Invite/Coin Number")
                        .GetComponent<TextMesh>().text = "+" + StagesParser.InviteReward;
                    Camera.main.GetComponent<MainScene>().friendsObjects[j].Find("FB Invite/Coin Number")
                        .GetComponent<TextMeshEffects>().RefreshTextOutline(false, false);
                    Camera.main.GetComponent<MainScene>().friendsObjects[j].Find("Friend").gameObject
                        .SetActive(false);
                }
...
//		FB.API("me/friends/?fields=installed,name,picture",Facebook.HttpMethod.GET,SviPrijateljiFacebookCallback);
        FB.API(
            "/fql?q=SELECT+uid,name,pic_square+FROM+user+WHERE+is_app_user=1+AND+uid+IN+(SELECT+uid2+FROM+friend+WHERE+uid1=me())",
            Facebook.HttpMethod.GET, SviPrijateljiFacebookCallback);
    }
    ...
    public void GetFacebookFriendPicture(string PrijateljevID)
    {
        if (Ulogovan)
        {
            ...
            FB.API(PrijateljevID + "/picture?redirect=true&height=64&type=normal&width=64", Facebook.HttpMethod.GET,
                FacebookFriendPictureCallback);
        }
    }
            ...
            if (MestoPozivanjaLogina == 1)
            {
                switch (BrojPrijatelja)
                {
                    case 1:
                        ...
                        GameObject.Find("Friends Level Win Picture 1").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[0].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 2:
                        //				FriendPic2=result.Texture;
                        GameObject.Find("Friends Level Win Picture 2").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[1].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 3:
                        //				FriendPic3=result.Texture;
                        GameObject.Find("Friends Level Win Picture 3").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[2].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 4:
                        //				FriendPic4=result.Texture;
                        GameObject.Find("Friends Level Win Picture 4").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[3].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 5:
                        //				FriendPic5=result.Texture;
                        GameObject.Find("Friends Level Win Picture 5").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[4].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 6:
                        //				FriendPic5=result.Texture;
                        GameObject.Find("Friends Level Win Picture 6").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[5].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 7:
                        //				FriendPic5=result.Texture;
                        GameObject.Find("Friends Level Win Picture 7").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[6].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 8:
                        //				FriendPic5=result.Texture;
                        GameObject.Find("Friends Level Win Picture 8").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[7].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 9:
                        //				FriendPic5=result.Texture;
                        GameObject.Find("Friends Level Win Picture 9").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[8].profilePicture;
                        WaitForFacebook = true;
                        break;

                    case 10:
                        //				FriendPic5=result.Texture;
                        GameObject.Find("Friends Level Win Picture 10").GetComponent<Renderer>().material.mainTexture =
                            ProfileSlikePrijatelja[9].profilePicture;
                        WaitForFacebook = true;
                        break;
                }
    ...
    public void GetFacebookGameAchievements()
    {
        FB.API(FB.AppId + "/achievements", Facebook.HttpMethod.GET,
            GetFacebookGameAchievementsCallback); // vraca sve achivmente iz igre
    }
    ...
    public void InicijalizujKorisnika(string KorisnikovID, int numCoins, int Score, string Jezik, int Banana,
        int Shield, int Magnet, int DoubleCoins, string UserSveKupovineHats, string UserSveKupovineShirts,
        string UserSveKupovineBackPacks) // poziva se samo jednom, prilikom prvog pokretanja aplikacije
    {
        if (FB.IsLoggedIn)
        {
            ...
            var saveTask = Korisnik.SaveAsync();
        ...
        if (FB.IsLoggedIn)
        {
            ...
            else
            {
                ...
                if (MestoPozivanjaLogina == 1)
                {
                    ...
                    StartCoroutine(StagesParser.Instance.moneyCounter(2000,
                        temp.transform.Find("Coins Number").GetComponent<TextMesh>(), true));
                    Invoke("SkloniCoinsReward", 1.2f);
                }
                else
                {
                    StartCoroutine(StagesParser.Instance.moneyCounter(StagesParser.LoginReward,
                        GameObject.Find("_GUI").transform.Find("INTERFACE HOLDER/_TopLeft/Coins/Coins Number")
                            .GetComponent<TextMesh>(), true));
                }
                ...
                InicijalizujKorisnika(User, StagesParser.currentMoney, StagesParser.currentPoints,
                    LanguageManager.chosenLanguage, StagesParser.currentBananas, StagesParser.powerup_shields,
                    StagesParser.powerup_magnets, StagesParser.powerup_doublecoins, StagesParser.svekupovineGlava,
                    StagesParser.svekupovineMajica, StagesParser.svekupovineLedja);
                InicijalizujScoreNaNivoima(StagesParser.StarsPoNivoima, StagesParser.PointsPoNivoima,
                    StagesParser.maxLevel, StagesParser.bonusLevels);
                GetFacebookFriendScores();
            }
        }
        else
    ...
    public void UpdateujPodatkeKorisnika(int BrojCoina, int Score, string Jezik, int Banana, int PowerMagnet,
        int PowerShield, int PowerDoubleCoins, string UserSveKupovineHats, string UserSveKupovineShirts,
        string UserSveKupovineBackPacks, int Ledja, int Glava, int Telo, bool Usi, bool Kosa, int NumberOfFriends)
    {
        if (FB.IsLoggedIn)
        {
...
//					}
    ...
    public void
        SacuvajBrojNovcica(
            int numCoins) // poziva se prilikom update-a broja coina, na kraju svakog nivo-a, ili izlaska iz igre
    {
        if (FB.IsLoggedIn)
        {
    ...
    public void SacuvajKupljeneStvari(string UserSveKupovineHats, string UserSveKupovineShirts,
        string UserSveKupovineBackPacks)
    {
        if (FB.IsLoggedIn)
        {
            ParseQuery<ParseObject> query = ParseObject.GetQuery("User");
...
//	}
    ...
    public void InicijalizujScoreNaNivoima(int[] NizBrojZvezdaPoNivou, int[] NizScorovaPoNivoima, int MaxNivo,
        string BonusLevels)
    {
        ...
        var saveTask = LevelScore.SaveAsync();
    }

    public void SacuvajScoreNaNivoima(int[] ScorePoNivoima, int[] BrojZvezdaPoNivoima, int TrenutniNivoIgraca,
        string BonusLevels)
    {
        if (FB.IsLoggedIn)
        {
...
//				GameObject.Find("Test").GetComponent<Renderer>().material.mainTexture= ProfileSlikePrijatelja[s];
            }
//			for(int i=0;i<100;i++)
//			{
//			}
        ...
        if (Ulogovan)
        {
            ...
            else
            {
                if (zavrsioUcitavanje)
                {
                    StagesParser.Instance
                        .UgasiLoading(); //ako se loguje kao novi user, ne treba da ulazi u CompareScores(), treba samo da ugasi loading
                    PlayerPrefs.SetInt("Logovan", 1);
                    if (MestoPozivanjaLogina == 1)
                        RefreshujScenu1PosleLogina();
                    else if (MestoPozivanjaLogina == 2)
