...
//	GameObject Pozadina;
    ...
    void Start()
    {
        ...
        transform.Find("Loading Animation Tip-s/Loading sa Tip-om/Loading Text").GetComponent<TextMesh>().text =
            LanguageManager.Loading;
        transform.Find("Loading Animation Tip-s/Loading sa Tip-om/Loading Text").GetComponent<TextMeshEffects>()
        ...
        else
        {
            ...
            GameObject Background =
                Instantiate(Resources.Load("LoadingBackground/BgP" +
                                           StagesParser.RedniBrojSlike[RandomBroj - 1].ToString())) as GameObject;
    ...
    IEnumerator LoadNextLevel()
    {
        ...
        //	Application.LoadLevelAsync("BOSS_LEVEL");
            ...
            Application.LoadLevelAsync(StagesParser
                .nivoZaUcitavanje); //BICE 9+StagesParser.currSetIndex KAD SE DODAJU NIVOI ZA OSTALA OSTRVA
    }


    public IEnumerator
        UcitanaScena(Camera camera, int skaliraj,
            float delay) // 1 - ucitan level, 2 - ucitana mapa, 3 - ucitana ostrva, 4 - ucitan main screen, 5 - Ucitana poslednja scena
    {
