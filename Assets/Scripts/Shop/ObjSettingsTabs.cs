...
public class ObjSettingsTabs : MonoBehaviour
{
    ...
    public float maxXPos = 40f; //max x position of the camera
    ...
    void Start()
    {
        ...
        if (!swipeCtrl) swipeCtrl = gameObject.AddComponent<SwipeControlSettingsTabs>();
        ...
        swipeCtrl.currentValue = obj.Length - 2; //current value set to max, so it starts from the end
        swipeCtrl.startValue = obj.Length - 2; //when Setup() is called it will animate from the end to the middle
