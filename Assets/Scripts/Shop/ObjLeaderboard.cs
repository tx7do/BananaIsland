...
public class ObjLeaderboard : MonoBehaviour
{
    ...
    public float maxXPos = 55f; //max x position of the camera
    ...
    void Start()
    {
        ...
        if (!swipeCtrl) swipeCtrl = gameObject.AddComponent<SwipeControlLeaderboard>();
        ...
        swipeCtrl.currentValue = obj.Length - 2; //current value set to max, so it starts from the end					bilo je 0
        swipeCtrl.startValue =
            obj.Length - 2; //when Setup() is called it will animate from the end to the middle		bilo je 1
...
//								obj[i].position = new Vector3(obj[i].position.x,minXPos + i * (xDist * swipeSmoothFactor) - swipeCtrl.smoothValue*swipeSmoothFactor*xDist , obj[i].position.z);
                ...
                //obj[i].position = new Vector3(obj[i].position.x,minXPos - i * (xDist * swipeSmoothFactor) + swipeCtrl.smoothValue*swipeSmoothFactor*xDist , obj[i].position.z);
                obj[i].position = new Vector3(obj[i].position.x,
