...
{
    public static bool controlEnabled = false; //turn this control on or off

    public bool
        skipAutoSetup =
            false; //If you set it up from another script you can skip the auto-setup! - Don't forget to call Setup() though!!

    public bool
        allowInput =
            true; //If you don't want to allow mouse or touch input and only want to use this control for animating a value, set this to false.

    public bool
        clickEdgeToSwitch =
            true; //When mouse-controlled, should a simple click on either side of the control increases/decreases the value by one?

    public float partWidth = 0f; // width

    private float
        partFactor = 1.0f; // calculated once in the beginning, so we don't have to do costly divisions every frame
    ...
    public Rect
        mouseRect; //where you can click to start the swipe movement (once you clicked you can drag outside as well)
    ...
    private int[]
        fingerStartArea = new int[5]; //set to 1 for each finger that starts touching the screen within our touchRect
    ...
    private float
        smoothDragOffset =
            0.2f; //how far (% of the width of one element) do we have to drag to set it to change currentValue?
    ...
    IEnumerator Start()
    {
        ...
        {
            Debug.LogWarning("You have enabled clickEdgeToSwitch, but it will not work because allowInput is disabled!",
                this);
        }
    ...
    public void Setup()
    {
        ...
        smoothValue =
            (float) currentValue; //Set smoothValue to the currentValue - tip: setting currentValue to -1 and startValue to 0 makes the first image appear at the start...
        ...
        if (leftEdgeRectForClickSwitch == new Rect(0, 0, 0, 0))
            CalculateEdgeRectsFromMouseRect(); //Only do this if not set in the inspector	
    ...
    void Update()
    {
        if (controlEnabled)
        {
            ...
            if (allowInput)
            {
                ...
                {
                    pos = new Vector3(Input.mousePosition.x, Input.mousePosition.y, 0.0f);
                    ...
                    if (mouseStartArea == 1)
                    {
                        ...
                        if (Input.GetMouseButtonUp(0))
                        {
                            ...
                            {
                                if (currentValue - (smoothValue + (smoothValue - GetAvgValue(prevSmoothValue))) >
                                    smoothDragOffset ||
                                    currentValue - (smoothValue + (smoothValue - GetAvgValue(prevSmoothValue))) <
                                    -smoothDragOffset)
                                {
                                    //dragged beyond dragOffset to the right
                                    currentValue =
                                        (int) Mathf.Round(smoothValue + (smoothValue - GetAvgValue(prevSmoothValue)));
...
//					pos = new Vector3(0, Screen.height - touch.position.y, 0.0f);
//					tPos = matrix.inverse.MultiplyPoint3x4(pos);		
//			
//					//BEGAN
...
//						}
//						//DRAGGING
//						smoothValue = smoothStartPos - tPos.y * partFactor; //print("smoothValue : " + smoothValue);
...
//				}
            ...
            if (!touched)
            {
                smoothValue = Mathf.SmoothDamp(smoothValue, (float) currentValue, ref xVelocity, 0.3f, maxSpeed,
                    Time.realtimeSinceStartup - realtimeStamp);
            }
