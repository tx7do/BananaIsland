...
public class ObjCustomizationShirts : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (!swipeCtrl) swipeCtrl = gameObject.AddComponent<SwipeControlCustomizationShirts>();
        ...
        swipeCtrl.startValue =
            obj.Length - 1; //when Setup() is called it will animate from the end to the middle		bilo je 0
        ...
        if (CustomizationShirts)
        {
            for (int i = 0; i < obj.Length; i++)
            {
                ...
                obj[i].position = new Vector3((xPosReal),
                    ...
                    obj[i].position.z);
                //obj[i].position = new Vector3((xPosReal - Mathf.Clamp(Mathf.Abs(i - swipeCtrl.smoothValue), 0.0f, 1.0f)),maxXPos - i * (xDist * swipeSmoothFactor) - swipeCtrl.smoothValue*swipeSmoothFactor*xDist , obj[i].position.z);

//				if(ShopManagerFull.AktivanCustomizationTab==1 && ShopManagerFull.AktivanItemSesir!=SwipeControlCustomization.currentValue)
//				{
//
//
//					ShopManagerFull.AktivanItemSesir=SwipeControlCustomization.currentValue;
...
//				}
                if (ShopManagerFull.AktivanCustomizationTab == 2 && ShopManagerFull.AktivanItemMajica !=
                    (swipeCtrl.maxValue - swipeCtrl.currentValue))
                {
                    ShopManagerFull.AktivanItemMajica =
                        swipeCtrl.maxValue - swipeCtrl.currentValue; //bilo je samo swipeCtrl.currentValue
                    ShopManagerFull.ShopObject.PreviewItem();
                }
//				else if(ShopManagerFull.AktivanCustomizationTab==3 && ShopManagerFull.AktivanItemRanac!=SwipeControlCustomization.currentValue)
//				{
//
//					ShopManagerFull.AktivanItemRanac=SwipeControlCustomization.currentValue;
//					ShopManagerFull.ShopObject.PreviewItem();
//				}
//				ShopManagerFull.ShopObject.PreviewItem();
