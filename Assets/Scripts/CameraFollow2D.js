#pragma strict
	...
	function Awake () 
	{
		...
		limitY = camera.ViewportToWorldPoint(Vector3.one*0.7f).y; 
		//transform.position.y = cameraTarget.transform.position.y;
		rotatingPlayer = GameObject.Find("RotatingPlayer").transform;
		cameraTarget.transform.position.y = camera.transform.position.y;
	}
	
	// Update is called once per frame
	function Update ()
	{
		if(camera.transform.position.y < cameraBottomLimit)
		{
			camera.transform.position.y = cameraBottomLimit;
			stopFollow = true;
		...
		if((cameraTarget.transform.position.y > camera.transform.position.y + borderY) || (rotatingPlayer.rotation.z*180/Mathf.PI > 5 && rotatingPlayer.rotation.z*180/Mathf.PI < 355))// && !grounded))// || (cameraTarget.transform.position.y > camera.transform.position.y && grounded))// (player.transform.position.y > limitY && grounded));
		{
		...
		if(moveUp)
		{
			camera.transform.position.y = Mathf.SmoothDamp(camera.transform.position.y, cameraTarget.transform.position.y, velocity.y, smoothTime, 15, Time.smoothDeltaTime);
			//transform.position.y = Mathf.Lerp(transform.position.y, cameraTarget.transform.position.y, smoothTime*Time.deltaTime*10);
		}
		if(Mathf.Abs(cameraTarget.transform.position.y - camera.transform.position.y) <= 0.1f && moveUp)
		{
			moveUp = false;
			moveDown = true;
		}
		if(cameraTarget.transform.position.y <= camera.transform.position.y)// - borderY)
		{
		...
		if(moveDown)
		{
			...
			camera.transform.position.y = cameraTarget.transform.position.y;
		...
		if(cameraTarget.transform.position.y >= camera.transform.position.y)
		{
			moveDown = false;
		}
		}
