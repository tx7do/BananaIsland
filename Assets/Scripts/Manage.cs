...
{
    [HideInInspector] public static int coinsCollected = 0;
    [HideInInspector] public int starsGained = 0;
    ...
    [HideInInspector] public bool PowerUp_doubleCoins = false;
    [HideInInspector] public bool PowerUp_shield = false;
    ...
    void Awake()
    {
        ...
        coinsCollectedText = transform.Find("Gameplay Scena Interface/_TopLeft/Coins/CoinsGamePlayText")
    ...
    void Start()
    {
        ...
        powerCard_CoinX2.parent.Find("CoinsHolder/Coins/Coins Number").GetComponent<TextMesh>().text =
            StagesParser.currentMoney.ToString();
        powerCard_CoinX2.parent.Find("CoinsHolder/Coins/Coins Number").GetComponent<TextMeshEffects>()
        ...
        powerCard_CoinX2.Find("Text/Cost Number").GetComponent<TextMesh>().text =
            StagesParser.cost_doublecoins.ToString();
        ...
        Win_CompletedScreenHolder.Find("FB Invite Large/Text/Number").GetComponent<TextMesh>().text =
            "+" + StagesParser.InviteReward;
        Win_CompletedScreenHolder.Find("FB Invite Large/Text/Number").GetComponent<TextMeshEffects>()
            .RefreshTextOutline(false, false);
        if (!StagesParser.bonusLevel)
        {
            Win_CompletedScreenHolder.Find("FB Share/Text/Number").GetComponent<TextMesh>().text =
                "+" + StagesParser.ShareReward;
            Win_CompletedScreenHolder.Find("FB Share/Text/Number").GetComponent<TextMeshEffects>()
                .RefreshTextOutline(false, false);
        }
        ...
        transform.Find("Gameplay Scena Interface/_TopLeft").position = new Vector3(
            guiCamera.ViewportToWorldPoint(Vector3.zero).x,
            transform.Find("Gameplay Scena Interface/_TopLeft").position.y,
            transform.Find("Gameplay Scena Interface/_TopLeft").position.z);
        transform.Find("Gameplay Scena Interface/_TopRight").position = new Vector3(
            guiCamera.ViewportToWorldPoint(Vector3.one).x,
            transform.Find("Gameplay Scena Interface/_TopRight").position.y,
            transform.Find("Gameplay Scena Interface/_TopRight").position.z);
    ...
    void Update()
    {
        if (pocniVreme)
        {
            ...
            {
                if (StagesParser.odgledaoTutorial == 0 && aktivnoVreme >= 35 &&
                    !postavljenFinish) //if(StagesParser.odgledaoTutorial == 0 && Time.time - goTrenutak >= 35 && !postavljenFinish)
                {
...
//				}
            ...
            //UBRZANJE
            if (aktivnoVreme - timeElapsed >= 20 &&
                stepenBrzine <= 7) //if(Time.time - timeElapsed >= 20 && stepenBrzine <= 7 && measureTime)
            {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            //if(shouldRaycast)
            {
                ...
                if (clickedItem == "RateButtonNO" || clickedItem == "RateButtonYES" || clickedItem == "WatchButtonNO" ||
                    clickedItem == "WatchButtonYES" || clickedItem == "Button Buy Banana"
                    || clickedItem == "Button Cancel" || clickedItem == "Button Play_Revive" ||
                    clickedItem == "Button Mapa_Win" || clickedItem == "Button Next_Win" ||
                    clickedItem == "Button Restart_Win"
                    || clickedItem == "Button Home_Failed" || clickedItem == "Button Mapa_Failed" ||
                    clickedItem == "Button Restart_Failed" || clickedItem == "Menu Button_Pause" ||
                    clickedItem == "Play Button_Pause" || clickedItem == "Restart Button_Pause")
                {
                ...
                if (clickedItem == releasedItem)
                {
                    if (releasedItem == "GO screen")
                    {
                        if (!StagesParser.bossStage)
                        {
                            ...
                            shouldRaycast = false;
                            if (!StagesParser.bonusLevel &&
                                MissionManager.missions[LevelFactory.level - 1].distance > 0 &&
                            ...
                            {
                                transform.Find("POWERS HOLDER").GetChild(0).GetComponent<Animator>()
                                    .Play("PowerUpDolazak");
                    ...
                    else if (releasedItem == "Button Play_Revive")
                    {
                        ...
                        if (StagesParser.currentBananas >= keepPlayingCount)
                        {
                            ...
                            Invoke("spustiPopup", 0.5f);
                            transform.Find("Keep Playing HOLDER").GetChild(0).GetComponent<Animator>()
                                .Play("ClosePopup");
                            StagesParser.currentBananas -= keepPlayingCount;
                            PlayerPrefs.SetInt("TotalBananas", StagesParser.currentBananas);
                            PlayerPrefs.Save();
                            transform.Find(
                                    "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Bananas/Banana Number")
                            ...
                            transform.Find(
                                    "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Bananas")
                        ...
                        if (StagesParser.currentMoney > StagesParser.bananaCost)
                        {
                            ...
                            transform
                                .Find(
                                    "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Bananas/Banana Number")
                                .GetComponent<TextMeshEffects>().RefreshTextOutline(false, true);
                            StartCoroutine(StagesParser.Instance.moneyCounter(-StagesParser.bananaCost,
                                transform.Find(
                                        "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/CoinsHolder/Coins/Coins Number")
                                    .GetComponent<TextMesh>(), true));
                        }
                        else
                        {
                            transform.Find(
                                    "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/CoinsHolder/Coins")
                                .GetComponent<Animation>().Play();
                        }
                    }
                    ...
                    {
                        GameObject.Find(releasedItem).transform.parent.parent.GetComponent<Animator>()
                            .Play("ClosePopup");
                        Time.timeScale = 1;
                        pauseEnabled = true;
                    }
                    else if (releasedItem == "Power_Double Coins Interface")
                    {
                        if (StagesParser.powerup_doublecoins > 0)
                        {
                            ...
                            powerCard_CoinX2.Find("Number").GetComponent<TextMesh>().text =
                        ...
                        else if (StagesParser.cost_doublecoins < StagesParser.currentMoney)
                        {
                            ...
                            PlayerPrefs.SetInt("TotalMoney", StagesParser.currentMoney);
                            PlayerPrefs.Save();
                            StartCoroutine(StagesParser.Instance.moneyCounter(-StagesParser.cost_doublecoins,
                                powerCard_Magnet.parent.Find("CoinsHolder/Coins/Coins Number").GetComponent<TextMesh>(),
                                true));
                            if (PlaySounds.soundOn)
                                PlaySounds.Play_CollectPowerUp();
                        }
                    ...
                    else if (releasedItem == "Power_Magnet Interface")
                    {
                        if (StagesParser.powerup_magnets > 0)
                        {
                            ...
                            powerCard_Magnet.Find("Number").GetComponent<TextMesh>().text =
                                StagesParser.powerup_magnets.ToString();
                            kupljenMagnet = true;
                            ApplyPowerUp(1);
                        ...
                        else if (StagesParser.cost_magnet < StagesParser.currentMoney)
                        {
                            ...
                            PlayerPrefs.SetInt("TotalMoney", StagesParser.currentMoney);
                            PlayerPrefs.Save();
                            StartCoroutine(StagesParser.Instance.moneyCounter(-StagesParser.cost_magnet,
                                powerCard_Magnet.parent.Find("CoinsHolder/Coins/Coins Number").GetComponent<TextMesh>(),
                                true));
                            if (PlaySounds.soundOn)
                                PlaySounds.Play_CollectPowerUp();
                    ...
                    else if (releasedItem == "Power_Shield Interface")
                    {
                        if (StagesParser.powerup_shields > 0)
                        {
                            ...
                            StagesParser.powerup_shields--;
                            powerCard_Shield.Find("Number").GetComponent<TextMesh>().text =
                                StagesParser.powerup_shields.ToString();
                            kupljenShield = true;
                            ApplyPowerUp(3);
                        ...
                        else if (StagesParser.cost_shield < StagesParser.currentMoney)
                        {
                            ...
                            PlayerPrefs.SetInt("TotalMoney", StagesParser.currentMoney);
                            PlayerPrefs.Save();
                            StartCoroutine(StagesParser.Instance.moneyCounter(-StagesParser.cost_shield,
                                powerCard_Magnet.parent.Find("CoinsHolder/Coins/Coins Number").GetComponent<TextMesh>(),
                                true));
                            if (PlaySounds.soundOn)
                                PlaySounds.Play_CollectPowerUp();
                    ...
                    else if (releasedItem.Contains("RateGame"))
                    {
                        ...
                        {
                            rateGameHolder.Find("RateGame" + i.ToString()).GetChild(0).GetComponent<Renderer>()
                                .enabled = true;
                        }
    ...
    {
        StartCoroutine(GameObject.Find("Menu_Pause").GetComponent<Animation>()
            .Play("ClickMenu", false, what => { helpBool = true; }));
        while (!helpBool)
        {
            yield return null;
        }
    ...
    {
        StartCoroutine(GameObject.Find("Restart_Pause").GetComponent<Animation>()
            .Play("ClickRestart", false, what => { helpBool = true; }));
        while (!helpBool)
        {
            yield return null;
        }
    ...
    IEnumerator unPause()
    {
        StartCoroutine(GameObject.Find("Play_Pause").GetComponent<Animation>()
            .Play("ClickMenu", false, what => { helpBool = true; }));
        while (!helpBool)
        {
            yield return null;
    ...
    void showFailedScreen()
    {
        ...
        FailedScreenHolder.parent.transform.position = new Vector3(guiCamera.transform.position.x,
            guiCamera.transform.position.y, FailedScreenHolder.transform.position.z);
    ...
    void ShowWinScreen()
    {
        ...
        if (StagesParser.bonusLevel)
        {
            transform.Find("Level Win_Lose SCENA/Popup za WIN HOLDER_BONUS/Popup za WIN/Header za win popup/Text/Level")
                .GetComponent<TextMesh>().text = LanguageManager.BonusLevel;
            transform.Find("Level Win_Lose SCENA/Popup za WIN HOLDER_BONUS/Popup za WIN/Header za win popup/Text/Level")
                .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
            transform.Find(
                    "Level Win_Lose SCENA/Popup za WIN HOLDER_BONUS/Popup za WIN/Header za win popup/Text/Completed")
                .GetComponent<TextMesh>().text = LanguageManager.Completed;
            transform
                .Find("Level Win_Lose SCENA/Popup za WIN HOLDER_BONUS/Popup za WIN/Header za win popup/Text/Completed")
                .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
            ...
            Win_CompletedScreenHolder.parent.position = new Vector3(guiCamera.transform.position.x,
                guiCamera.transform.position.y, Win_CompletedScreenHolder.transform.position.z);
        ...
        {
            Win_CompletedScreenHolder.Find("Popup za WIN/Header za win popup/Text/Level No").GetComponent<TextMesh>()
                .text = (StagesParser.currStageIndex + 1).ToString();
            Win_CompletedScreenHolder.Find("Popup za WIN/Header za win popup/Text/Level No")
                .GetComponent<TextMeshEffects>().RefreshTextOutline(false, true);
    ...
    IEnumerator waitForStars()
    {
        if (MissionManager.points3Stars > 0)
            Win_CompletedScreenHolder.Find("Popup za WIN/ProgressBarHolder/3StarsPtsText").GetComponent<TextMesh>()
                .text = MissionManager.points3Stars.ToString();
        else //Tutorial Level
            Win_CompletedScreenHolder.Find("Popup za WIN/ProgressBarHolder/3StarsPtsText").GetComponent<TextMesh>()
                .text = "500";
        Win_CompletedScreenHolder.Find("Popup za WIN/ProgressBarHolder/3StarsPtsText").GetComponent<TextMeshEffects>()
            .RefreshTextOutline(false, true);
        ...
        TextMesh scoreDisplay = Win_CompletedScreenHolder
            .Find("Popup za WIN/Polje za unos poena Na Level WIN/Points Number level win").GetComponent<TextMesh>();
        ...
        while (progressBarPivot.localScale.x < targetScaleX || currentProgressBarPoints <= pointsForDisplay)
        {
            ...
            progressBarPivot.localScale =
                new Vector3(Mathf.MoveTowards(progressBarPivot.localScale.x, targetScaleX, Time.deltaTime / 2),
                    progressBarPivot.localScale.y, progressBarPivot.localScale.z);
        }
...
//			}
    ...
    public void ApplyPowerUp(int x)
    {
        ...
        else if (x == -1)
        {
            ...
            CancelInvoke("pustiAnimacijuBlinkanja");
            transform.Find("Gameplay Scena Interface/_TopLeft/Magnet Icon Holder/Icon Animation").localScale =
                new Vector3(0.0001f, 0.0001f, 1);
        }
        else if (x == 2)
        {
            if (!PowerUp_doubleCoins)
            {
                transform.Find("Gameplay Scena Interface/_TopLeft/DoubleCoins Icon Holder").GetComponent<Animation>()
                    .Play();
        ...
        else if (x == -2)
        {
            ...
            transform.Find("Gameplay Scena Interface/_TopLeft/DoubleCoins Icon Holder/Icon Animation").localScale =
                new Vector3(0.0001f, 0.0001f, 1);
        }
        else if (x == 3)
        {
    ...
    void pustiAnimacijuBlinkanja()
    {
        ...
        {
            transform.Find("Gameplay Scena Interface/_TopLeft/Magnet Icon Holder").GetComponent<Animation>()
                .Play("PowerUp Icon Disappear NEW");
    ...
    void ShowKeepPlayingScreen()
    {
        measureTime = false;
        Transform temp =
            transform.Find("Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing");
        ...
        temp.Find("CoinsHolder/Coins/Coins Number").GetComponent<TextMesh>().text =
            StagesParser.currentMoney.ToString();
    ...
    {
        transform.Find("LOADING HOLDER NEW/Loading Animation Vrata").GetComponent<Animator>()
            .Play("Loading Zidovi Dolazak");
    ...
    void refreshText()
    {
        ...
        transform.Find("GO screen/GO screen Text").GetComponent<TextMeshEffects>()
            .RefreshTextOutline(true, true, false);
        transform.Find(
                "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Button Buy Banana/BuyText")
            .GetComponent<TextMesh>().text = LanguageManager.Buy;
        transform
            .Find(
                "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Button Buy Banana/BuyText")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find(
                "Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Text/Keep Playing")
            .GetComponent<TextMesh>().text = LanguageManager.KeepPlaying;
        transform
            .Find("Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Text/Keep Playing")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("Level Win_Lose SCENA/Popup za LOSE HOLDER/Popup za LOSE/Header za LOSE popup/Text/Level Failed")
            .GetComponent<TextMesh>().text = LanguageManager.LevelFailed;
        transform.Find("Level Win_Lose SCENA/Popup za LOSE HOLDER/Popup za LOSE/Header za LOSE popup/Text/Level Failed")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        Win_CompletedScreenHolder.Find("FB Invite Large/Text/Invite").GetComponent<TextMesh>().text =
            LanguageManager.Invite;
        Win_CompletedScreenHolder.Find("FB Invite Large/Text/Invite").GetComponent<TextMeshEffects>()
        ...
        if (!StagesParser.bonusLevel)
        {
            ...
            Win_CompletedScreenHolder.Find("FB Share/Text/Share").GetComponent<TextMeshEffects>()
                .RefreshTextOutline(true, true);
        }

        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 1 HOLDER/FB/Fb Invite 1")
            .GetComponent<TextMesh>().text = LanguageManager.Invite;
        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 2 HOLDER/FB/Fb Invite 1")
            .GetComponent<TextMesh>().text = LanguageManager.Invite;
        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 3 HOLDER/FB/Fb Invite 1")
            .GetComponent<TextMesh>().text = LanguageManager.Invite;
        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 4 HOLDER/FB/Fb Invite 1")
            .GetComponent<TextMesh>().text = LanguageManager.Invite;
        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 5 HOLDER/FB/Fb Invite 1")
            .GetComponent<TextMesh>().text = LanguageManager.Invite;
        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 1 HOLDER/FB/Fb Invite 1")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 2 HOLDER/FB/Fb Invite 1")
        ...
        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win 5 HOLDER/FB/Fb Invite 1")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        Win_CompletedScreenHolder.Find("Popup za WIN/Header za win popup/Text/Level").GetComponent<TextMesh>().text =
            LanguageManager.Level;
        Win_CompletedScreenHolder.Find("Popup za WIN/Header za win popup/Text/Level").GetComponent<TextMeshEffects>()
            .RefreshTextOutline(true, true);
        Win_CompletedScreenHolder.Find("Popup za WIN/Header za win popup/Text/Completed").GetComponent<TextMesh>()
            .text = LanguageManager.Completed;
        Win_CompletedScreenHolder.Find("Popup za WIN/Header za win popup/Text/Completed")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("PAUSE HOLDER/AnimationHolderGlavni/AnimationHolder/PAUSE PopUp/PauseText")
            .GetComponent<TextMesh>().text = LanguageManager.Pause;
        transform.Find("PAUSE HOLDER/AnimationHolderGlavni/AnimationHolder/PAUSE PopUp/PauseText")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("RATE HOLDER/AnimationHolderGlavni/AnimationHolder/RATE Popup/Rate").GetComponent<TextMesh>()
            .text = LanguageManager.RateThisGame;
        transform.Find("RATE HOLDER/AnimationHolderGlavni/AnimationHolder/RATE Popup/Rate")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("RATE HOLDER/AnimationHolderGlavni/AnimationHolder/RATE Popup/Text 1").GetComponent<TextMesh>()
            .text = LanguageManager.HowWouldYouRate;
        transform.Find("RATE HOLDER/AnimationHolderGlavni/AnimationHolder/RATE Popup/Text 1")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/Free Coins")
            .GetComponent<TextMesh>().text = LanguageManager.FreeCoins;
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/Free Coins")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/WatchButtonNO/Text")
            .GetComponent<TextMesh>().text = LanguageManager.No;
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/WatchButtonNO/Text")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/WatchButtonYES/Text")
            .GetComponent<TextMesh>().text = LanguageManager.Yes;
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/WatchButtonYES/Text")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/WatchVideoText")
            .GetComponent<TextMesh>().text = LanguageManager.WatchVideo;
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/WatchVideoText")
            .GetComponent<TextMeshEffects>().RefreshTextOutline(true, true);
        transform.Find("WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/NotAvailableText")
    ...
    void getFriendsScoresOnLevel(int level)
    {
        ...
        for (int i = 0; i < scoresAndIndexes.Count; i++)
        {
            ...
            if (i < 5)
            {
                ...
                {
                    trenutniPrijatelj =
                        Win_CompletedScreenHolder.Find("Friends FB level WIN/Friends Level Win " + redosled +
                                                       " HOLDER");
                    ...
                    trenutniPrijatelj.Find("Friends Level Win " + redosled + "/Friends Level Win Picture " + redosled)
                        ...
                        .ProfileSlikePrijatelja[scoresAndIndexes[i].index].profilePicture;
                    trenutniPrijatelj
                ...
                else
                {
                    if (redosled <= 5)
                    {
                        Win_CompletedScreenHolder
                            .Find("Friends FB level WIN/Friends Level Win " + redosled + " HOLDER/FB").gameObject
                            .SetActive(true);
                        Win_CompletedScreenHolder
                            .Find("Friends FB level WIN/Friends Level Win " + redosled + " HOLDER/Friends Level Win " +
                                  redosled).gameObject.SetActive(false);
                    }
                }
            }
        ...
        if (scoresAndIndexes.Count < 5)
        {
            ...
            {
                Win_CompletedScreenHolder
                    .Find("Friends FB level WIN/Friends Level Win " + i + " HOLDER/Friends Level Win " + i).gameObject
                    .SetActive(false);
            }
        }
        ...
        if (!korisnik)
        {
            ...
            trenutniPrijatelj.Find("Friends Level Win 5/Friends Level Win Picture 5").GetComponent<Renderer>().material
                    .mainTexture =
                FacebookManager.ListaStructPrijatelja[scoresAndIndexes[pozicijaKorisnika - 1].index].profilePicture;
...
//			else
    ...
    void WatchVideoCallback()
    {
        ...
        PlayerPrefs.SetInt("TotalMoney", StagesParser.currentMoney);
        PlayerPrefs.Save();
        StartCoroutine(StagesParser.Instance.moneyCounter(watchVideoReward,
            transform.Find(
                    "WATCH VIDEO HOLDER/AnimationHolderGlavni/AnimationHolder/WATCH VIDEO Popup/Coins/Coins Number")
                .GetComponent<TextMesh>(), true));
        StagesParser.ServerUpdate = 1;
    }
