...
//				}
				...
				if(clickedItem == releasedItem)
				{
					...
					else if(releasedItem == "Button Play_Revive")
					{
						...
						if(StagesParser.currentBananas >= keepPlayingCount)
						{
							...
							transform.Find("Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/Bananas").animation.Play();
					}
					...
					else if(releasedItem.Equals("Button Buy Banana"))
					{
						...
						{
							transform.Find("Keep Playing HOLDER/AnimationHolderGlavni/AnimationHolder/Popup za Keep Playing/CoinsHolder/Coins").animation.Play();
						}
					}
					...
					else if(releasedItem == "Power_Double Coins Interface")
					{
						...
						else if(StagesParser.cost_doublecoins < StagesParser.currentMoney)
						{
							...
							powerCard_CoinX2.parent.Find("CoinsHolder/Coins").animation.Play();
					}
					else if(releasedItem == "Power_Magnet Interface")
					{
						...
						else if(StagesParser.cost_magnet < StagesParser.currentMoney)
						{
							...
							powerCard_Magnet.parent.Find("CoinsHolder/Coins").animation.Play();
					}
					else if(releasedItem == "Power_Shield Interface")
					{
						...
						else if(StagesParser.cost_shield < StagesParser.currentMoney)
						{
							...
							powerCard_Shield.parent.Find("CoinsHolder/Coins").animation.Play();
					}
					...
					else if(releasedItem.Contains("RateGame"))
					{
						...
						{
							rateGameHolder.GetChild(i).GetChild(0).renderer.enabled = false;
						}

						for(int i=0;i<starsGiven;i++)
						{
							rateGameHolder.Find("RateGame"+i.ToString()).GetChild(0).renderer.enabled = true;
						}
						if(starsGiven > 3)
						{
	...
	{
		StartCoroutine( GameObject.Find("Menu_Pause").animation.Play("ClickMenu", false, what => {helpBool=true;}) );
		while(!helpBool)
		{
			yield return null;
		}
	...
	{
		StartCoroutine( GameObject.Find("Restart_Pause").animation.Play("ClickRestart", false, what => {helpBool=true;}) );
		while(!helpBool)
		{
			yield return null;
		}
	...
	IEnumerator unPause()
	{
		StartCoroutine( GameObject.Find("Play_Pause").animation.Play("ClickMenu", false, what => {helpBool=true;}) );
		while(!helpBool)
		{
			yield return null;
		}
	...
	IEnumerator WaitForSave()
	{
		...
		a1.collider.enabled = true;
		a2.collider.enabled = true;
		a3.collider.enabled = true;
...
//			}
	...
	public void ApplyPowerUp(int x)
	{
		if(x == 1)
		{
			...
			transform.Find("Gameplay Scena Interface/_TopLeft/Magnet Icon Holder").animation.Play();
		...
		else if(x == 2)
		{
			if(!PowerUp_doubleCoins)
			{
				transform.Find("Gameplay Scena Interface/_TopLeft/DoubleCoins Icon Holder").animation.Play();
	...
	void pustiAnimacijuBlinkanja()
	{
		...
		{
			transform.Find("Gameplay Scena Interface/_TopLeft/Magnet Icon Holder").animation.Play("PowerUp Icon Disappear NEW");
	...
	void getFriendsScoresOnLevel(int level)
	{
		...
		for(int i=0;i<scoresAndIndexes.Count;i++)
		{
			...
			if(i<5)
			{
				if(scoresAndIndexes[i].score > 0 || FacebookManager.ListaStructPrijatelja[scoresAndIndexes[i].index].PrijateljID==FacebookManager.User)
				{
					...
					trenutniPrijatelj.Find("Friends Level Win " + redosled+"/Friends Level Win Picture " + redosled).renderer.material.mainTexture = FacebookManager.ProfileSlikePrijatelja[scoresAndIndexes[i].index].profilePicture;
		...
		if(!korisnik)
		{
			...
			trenutniPrijatelj.Find("Friends Level Win 5/Friends Level Win Picture 5").renderer.material.mainTexture = FacebookManager.ListaStructPrijatelja[scoresAndIndexes[pozicijaKorisnika-1].index].profilePicture;
