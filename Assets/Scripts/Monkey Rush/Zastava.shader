...
Shader "ZaMajmuna/Zastava"
{
 
Properties
{
    _Color ("Boja", Color) = (1,1,1,1)
    _MainTex ("Texture", 2D) = "white" {}
    _WaveSpeed ("Brzina vijorenja zastave", float) = 50.0
    _WaveX ("X", float) = 2.0
    _WaveY ("Y", float) = 2.0
    _WaveZ ("Z", float) = 2.0
}
...
Blend SrcAlpha OneMinusSrcAlpha
    Pass
    {
      ...
      v2f vert (appdata v) {
        ...
        v.vertex.x+=sin(t*1.45+sinOff)*fx*0.5*_WaveX;
        v.vertex.y=sin(t*3.12+sinOff)*fx*0.5-fy*0.9*_WaveY;
        v.vertex.z-=sin(t*2.2+sinOff)*fx*0.2*_WaveZ;
