...
public class CameraFollow2D_new : MonoBehaviour
{
    ...
    [HideInInspector] public Transform rotatingPlayer;
    ...
    [HideInInspector] public float upperLimit = 0.7f;
    ...
    void Awake()
    {
        ...
        cameraTarget.transform.position = new Vector3(cameraTarget.transform.position.x,
            Camera.main.transform.position.y, cameraTarget.transform.position.z);
    ...
    void Start()
    {
        ...
        if (Camera.main.aspect < 1.5f)
        {
            Camera.main.transform.position =
                new Vector3(Camera.main.transform.position.x, 7.5f, Camera.main.transform.position.z);
            cameraTarget.transform.position = new Vector3(cameraTarget.transform.position.x,
                Camera.main.transform.position.y, cameraTarget.transform.position.z);
            //borderY = 15f;
        ...
        {
            Camera.main.transform.position =
                new Vector3(Camera.main.transform.position.x, 8.5f, Camera.main.transform.position.z);
            cameraTarget.transform.position = new Vector3(cameraTarget.transform.position.x,
                Camera.main.transform.position.y, cameraTarget.transform.position.z);
        }
    }
...
//		}
        if (!stopFollow)
        {
            if (cameraFollowX == true)
            {
                ...
                thisTransform.position = Vector3.Lerp(thisTransform.position,
                    new Vector3(cameraTarget.transform.position.x, thisTransform.position.y, thisTransform.position.z),
                    5 * Time.deltaTime);
            }
            ...
            if (cameraFollowY == true)
            {
                thisTransform.position = Vector3.SmoothDamp(thisTransform.position,
                    new Vector3(thisTransform.position.x, cameraTarget.transform.position.y, thisTransform.position.z),
                    ref velocity, smoothTime);
            }

            if (!cameraFollowY && cameraFollowHeight)
            {
                Camera.main.transform.position = new Vector3(Camera.main.transform.position.x, cameraHeight,
            ...
            if ((playerController.state == MonkeyController2D.State.jumped && player.transform.position.y >
                    Camera.main.ScreenToWorldPoint(new Vector3(0, Screen.height * upperLimit, 0)).y) ||
                (playerController.state != MonkeyController2D.State.jumped && player.transform.position.y >
                    Camera.main.ScreenToWorldPoint(new Vector3(0, Screen.height * 0.25f, 0)).y))
            {
                moveUp = true;
            ...
            if (moveUp)
            {
                if (playerController.heCanJump)
                    thisTransform.position = Vector3.SmoothDamp(thisTransform.position,
                        new Vector3(thisTransform.position.x, cameraTarget.transform.position.y,
                            thisTransform.position.z), ref velocity, smoothTime, 2000 * Time.deltaTime,
                        Time.smoothDeltaTime); // BILO JE 1000 * Time.deltaTime
            }

            if (playerController.state == MonkeyController2D.State.jumped &&
                Mathf.Abs(cameraTarget.transform.position.y - Camera.main.transform.position.y) <= 0.1f && moveUp)
            {
                moveUp = false;
                //moveDown = true;
            }
            ...
            {
                if (!transition)
                    thisTransform.position = new Vector3(thisTransform.position.x, cameraTarget.transform.position.y,
