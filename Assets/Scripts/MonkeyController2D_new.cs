...
public class MonkeyController2D_new : MonoBehaviour {
	...
	void OnTriggerExit2D(Collider2D col)
	{
		if(col.tag == "Footer")
		{
			if(collider2D.isTrigger && transform.position.y > col.transform.position.y)
			{
	...
	void OnTriggerEnter2D(Collider2D col)
	{
		if(state != State.completed || state != State.wasted)
		{
			...
			{
				if(transform.position.y + 0.5f > col.transform.position.y && triggerCheckDownTrigger && triggerCheckDownBehind && collider2D.isTrigger)
				{
					collider2D.isTrigger = false;
				}
