...
public class MonkeyController2D_new : MonoBehaviour
{
    ...
    void Update()
    {
        hit = Physics2D.Linecast(transform.position + new Vector3(0.8f, 2.5f, 0),
        ...
        triggerCheckDownTrigger = Physics2D.Linecast(transform.position + new Vector3(0.8f, 2.5f, 0),
            transform.position + new Vector3(0.8f, -4.5f, 0),
            1 << LayerMask.NameToLayer("Ground") | 1 << LayerMask.NameToLayer("Platform"));
            ...
            if (KontrolisaniSkok)
            {
                if (Input.GetMouseButton(0) && jumpControlled)
                {
                    GetComponent<Rigidbody2D>().velocity = new Vector2(GetComponent<Rigidbody2D>().velocity.x,
                        GetComponent<Rigidbody2D>().velocity.y + korakce);
                    if (korakce > 0)
                        korakce -= 0.085f;
                }
    ...
    void FixedUpdate()
    {
            ...
            if (jump)
            {
                ...
                if (GetComponent<Rigidbody2D>().velocity.y < maxSpeedY)
                {
                    ...
                    GetComponent<Rigidbody2D>()
                        .AddForce(new Vector2(GetComponent<Rigidbody2D>().velocity.x, 2500)); //promenjeno sa 0
                }
        ...
        else if (state == State.jumped) // ************* Player is in the air
        {
            if (swoosh)
            {
                ...
                GetComponent<Rigidbody2D>()
                    .AddForce(new Vector2(GetComponent<Rigidbody2D>().velocity.x, -3000)); //promenjeno sa 0
                swoosh = false;
            }
        ...
        else if (state == State.climbUp) // ************* Player is climbing
        {
            ...
            else if (jump)
            {
                GetComponent<Rigidbody2D>().velocity = Vector2.zero;
                GetComponent<Rigidbody2D>()
                    .AddForce(new Vector2(GetComponent<Rigidbody2D>().velocity.x, jumpForce)); //promenjeno sa 0
                hasJumped = true;
                jump = false;
            }
            else if (jumpControlled)
            {
                ...
                if (GetComponent<Rigidbody2D>().velocity.y < maxSpeedY)
                {
                    //GetComponent<Rigidbody2D>().velocity = Vector2.zero;
                    GetComponent<Rigidbody2D>()
                        .AddForce(new Vector2(GetComponent<Rigidbody2D>().velocity.x, tempForce)); // promenjeno sa 0
        ...
        if (state != State.completed || state != State.wasted)
        {
            ...
            else if (col.gameObject.tag == "Footer")
            {
                ...
                if (state == State.jumped)
                {
                    ...
                    if (GetComponent<Collider2D>().isTrigger == false &&
                        triggerCheckDownTrigger) // DODATO JE "&& triggerCheckDownTrigger" i promenjen je x u triggerCheckDown sa 1.1 na 0.8, zbog provere ako si skocio bas na samu ivicu, on udje u running ali odmah padne
                    {
                        if (startSpustanje)
                        {
                            startSpustanje = false;
            ...
            else if (col.gameObject.tag == "Enemy")
            {
                ...
                if (activeShield)
                {
                    ...
                    if (state != State.running)
                        GetComponent<Rigidbody2D>()
                            .AddForce(new Vector2(GetComponent<Rigidbody2D>().velocity.x, 1100)); //promenjeno sa 0
                }
                else if (!killed)
                {
                    if (state == State.running)
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        if (state != State.completed || state != State.wasted)
        {
            ...
            {
                if (transform.position.y + 0.5f > col.transform.position.y && triggerCheckDownTrigger &&
