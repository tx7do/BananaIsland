...
public class CameraFollow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            float step = Mathf.SmoothDamp(thisTransform.position.x, cameraTarget.transform.position.x, ref velocity.x,
                smoothTime);
        ...
        if (cameraFollowY == true)
        {
            //if(monkeyControll != null)
            {
                ...
                {
                    float step = Mathf.SmoothDamp(thisTransform.position.y, cameraTarget.transform.position.y,
        ...
        while (i < 1)
        {
            ...
            if (changeHeight)
            {
                thisTransform.position = new Vector3(thisTransform.position.x,
                    Mathf.MoveTowards(thisTransform.position.y, cameraTarget.transform.position.y, i),
