...
//		}
		...
		if(state == State.jumped || state == State.climbUp)
		{
				...
				if(SlideNaDole)
				{
					if((startY - endY) > ( (1f/4f*Camera.main.orthographicSize)*(Screen.height/(2*Camera.main.orthographicSize))))
					{
						...
						if(!Physics2D.Linecast(groundCheck.position, groundCheck.position - Vector3.up*20, whatIsGround))
						{
							...
							//majmun.Find("PorinceGorilla_LP").renderer.material.color = Color.red;
			...
			if(KontrolisaniSkok)
			{
				...
				{
					//rigidbody2D.velocity = new Vector2(rigidbody2D.velocity.x, rigidbody2D.velocity.y + korakce);
...
//						korakce -= Time.deltaTime * 5.5f;
			...
			if(KontrolisaniSkok)
			{
				...
				if((Input.GetMouseButtonDown(0) && Input.mousePosition.x > povrsinaZaClick) || Input.GetKeyDown(KeyCode.Space))
				{
					...
					{	
						...
						startVelY = rigidbody2D.velocity.y;
						korakce = 3;
	...
	void FixedUpdate ()
	{
		...
		else if(state == State.jumped) 	// ************* Player is in the air
		{
			...
			// ----------------------------------------------------------
			if(rigidbody2D.velocity.y < -0.01f && !triggerCheckDown)
			{
		...
		else if(state == State.climbUp)	// ************* Player is climbing
		{
			...
			else if(jumpControlled)
			{
				...
				{
					//rigidbody2D.velocity = Vector2.zero;
					//rigidbody2D.AddForce(new Vector2(rigidbody2D.velocity.x, tempForce)); // promenjeno sa 0
	...
	void OnCollisionEnter2D(Collision2D col)
	{
		if(state != State.completed || state != State.wasted)
		{
			...
			else if(col.gameObject.tag == "Enemy")
			{
				...
				if(activeShield)
				{
					col.transform.collider2D.enabled = false;
					activeShield = false;
	...
	void OnTriggerStay2D(Collider2D col)
	{
		if(col.tag == "GrabLedge")
			rigidbody2D.velocity = new Vector2(0, Mathf.MoveTowards(rigidbody2D.velocity.y,0,0.2f));
	}
