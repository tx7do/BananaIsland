...
public class MonkeyController2D_bekapce : MonoBehaviour
{
    ...
    bool downHit = false;
    [HideInInspector] public bool zgaziEnemija = false;
    [HideInInspector] public bool killed = false;

    [HideInInspector] public bool stop = false;
    bool triggerCheckDown = false;
    [HideInInspector] public bool triggerCheckDownTrigger = false;
    [HideInInspector] public bool triggerCheckDownBehind = false;
    ...
    [HideInInspector] public float collisionAngle = 0;
    ...
    [HideInInspector] public GameObject cameraTarget_down;
    ...
    [HideInInspector] public float startSpeedX;
    [HideInInspector] public float startJumpSpeedX;
    ...
    [HideInInspector] public string lastPlayedAnim;
    bool helpBool;
    ...
    [HideInInspector] public bool lijana = false;
    Transform grabLianaTransform;
    [HideInInspector] public bool heCanJump = true;
    ...
    void Update()
    {
        ...
        hit = Physics2D.Linecast(transform.position + new Vector3(0.8f, 0f, 0),
            transform.position + new Vector3(0.8f, -15.5f, 0),
...
//		}
        ...
        if (startSpustanje)
        {
            if (startPenjanje)
            {
                ...
                cameraTarget_down.transform.position = new Vector3(cameraTarget.transform.position.x,
                    pocetniY_spustanje, cameraTarget_down.transform.position.z);
...
//		}
        ...
        //grounded = Physics2D.Linecast(transform.position+Vector3.one, groundCheck.position + Vector3.down/2, whatIsGround);
        CheckWallHitNear = Physics2D.Linecast(transform.position + new Vector3(0.8f, 2.5f, 0),
            transform.position + new Vector3(3.5f, 2.5f, 0),
            (1 << LayerMask.NameToLayer("WallHit"))); // | (1 << LayerMask.NameToLayer("Ground")));
        CheckWallHitNear_low = Physics2D.Linecast(transform.position + new Vector3(0.8f, 0f, 0),
            transform.position + new Vector3(3.5f, 0f, 0), (1 << LayerMask.NameToLayer("WallHit")));
        ...
        triggerCheckDown = Physics2D.Linecast(transform.position + new Vector3(0.8f, 2.5f, 0),
            transform.position + new Vector3(0.8f, -0.5f, 0),
            1 << LayerMask.NameToLayer("Ground") | 1 << LayerMask.NameToLayer("Platform"));
        triggerCheckDownTrigger = Physics2D.Linecast(transform.position + new Vector3(0.8f, 2.5f, 0),
            transform.position + new Vector3(0.8f, -4.5f, 0), /*1 << LayerMask.NameToLayer("Ground") | */
            1 << LayerMask.NameToLayer("Platform"));
        triggerCheckDownBehind = Physics2D.Linecast(transform.position + new Vector3(-0.8f, 2.5f, 0),
            transform.position + new Vector3(-0.8f, -4.5f, 0), 1 << LayerMask.NameToLayer("Platform"));
        proveriTerenIspredY = Physics2D.Linecast(transform.position + new Vector3(4.4f, 1.2f, 0),
            transform.position + new Vector3(4.4f, -3.2f, 0),
            1 << LayerMask.NameToLayer("Ground") | 1 << LayerMask.NameToLayer("Platform"));
        downHit = Physics2D.Linecast(transform.position + new Vector3(0.2f, 0.1f, 0),
            transform.position + new Vector3(0.2f, -0.65f, 0),
            1 << LayerMask.NameToLayer("Ground") | 1 << LayerMask.NameToLayer("Platform"));
        spustanjeRastojanje = Physics2D.Linecast(transform.position + new Vector3(2.3f, 1.25f, 0),
            transform.position + new Vector3(2.3f, -Camera.main.orthographicSize, 0),
            1 << LayerMask.NameToLayer("Ground") | 1 << LayerMask.NameToLayer("Platform"));
        ...
        if (state == State.jumped || state == State.climbUp)
        {
            if (DupliSkok) //Opcija za dupli skok
            {
                ...
                // INPUT WITH TOUCHES
                         ...
                         Input.GetTouch(1).phase ==
                         TouchPhase.Began) // && Input.GetTouch(1).position.x > Screen.width/2)
                {
            ...
            if (SlideNaDole || Glide)
            {
                if ((Input.GetMouseButtonDown(0) ||
                     Input.GetKeyDown(KeyCode.Space))) //  && Input.mousePosition.x < Screen.width/2)
                {
                    ...
                    canGlide = true;
                          ...
                          Input.GetKeyUp(KeyCode.Space))) // && Input.mousePosition.x < Screen.width/2) 
                {
                    startY = endY = 0;
                    GetComponent<Rigidbody2D>().drag = 0;
                    animator.SetBool("Glide", false);
            ...
            if ((Input.GetMouseButton(0))) // && Input.mousePosition.x < Screen.width/2)
            {
                ...
                if (SlideNaDole)
                {
                    if ((startY - endY) > ((1f / 4f * Camera.main.orthographicSize) *
                                           (Screen.height / (2 * Camera.main.orthographicSize))))
                    {
                        //GetComponent<Rigidbody2D>().gravityScale = 20;
                        if (!Physics2D.Linecast(groundCheck.position, groundCheck.position - Vector3.up * 20,
                                whatIsGround))
                        {
                            powerfullImpact = true;
                            //majmun.Find("PorinceGorilla_LP").GetComponent<Renderer>().material.color = Color.red;
                ...
                if (Glide && canGlide)
                {
                    if (!disableGlide)
                    {
                        ...
                        GetComponent<Rigidbody2D>().velocity =
                            new Vector2(maxSpeedX, GetComponent<Rigidbody2D>().velocity.y);
...
//						korakce -= Time.deltaTime * 5.5f;
                    //Debug.Log("nju: " + Time.deltaTime);
                }

                if ((Input.GetMouseButtonUp(0) || Time.time - duzinaPritiskaZaSkok > 1f) &&
                    jumpControlled) //njanjanjanja 0.35 //zadnje 0.3
                {
                    //Debug.Log("Gotovo!!!");
        ...
        if (state == State.wallhit)
        {
            if (KontrolisaniSkok)
            {
                    ...
                    Input.GetKeyDown(KeyCode.Space))
                {
                    ...
                    if (RaycastFunction(Input.mousePosition) != "Pause" &&
                        RaycastFunction(Input.mousePosition) != "HoleButtonsPlay_Pause" &&
                        RaycastFunction(Input.mousePosition) != "Buy Button" &&
                        RaycastFunction(Input.mousePosition) != "ButtonMain_NoMoreLives" &&
                        RaycastFunction(Input.mousePosition) != "HoleButtonsRestart_Pause" &&
        ...
        if (state == State.lijana)
        {
            ...
            if (Input.GetMouseButtonDown(0) && Input.mousePosition.x > povrsinaZaClick)
            {
                if (heCanJump && RaycastFunction(Input.mousePosition) != "ButtonPause" &&
                    RaycastFunction(Input.mousePosition) != "PauseHolePlay" &&
                    RaycastFunction(Input.mousePosition) != "PauseHoleShop" &&
                    RaycastFunction(Input.mousePosition) != "PauseHoleFreeCoins" &&
                    RaycastFunction(Input.mousePosition) != "PowersCardShield" &&
                    RaycastFunction(Input.mousePosition) != "PowersCardMagnet" &&
                    RaycastFunction(Input.mousePosition) != "PowersCardCoinx2" &&
        ...
        //Debug.Log("maxSpeedX: " + maxSpeedX + ", speedX: " + GetComponent<Rigidbody2D>().velocity.x + ", speedY: " + GetComponent<Rigidbody2D>().velocity.y);
        //Debug.Log("grounded: " + grounded + ", state: " + state);
    }
    ...
    void FixedUpdate()
    {
        ...
        //Debug.Log ("grounded: " + grounded + ", in air: " + inAir + ", side hit: " + sideHit + ", v: " + GetComponent<Rigidbody2D>().velocity.y + ", jump: " + jump + ", gravity: " + GetComponent<Rigidbody2D>().gravityScale);
        //Debug.Log(state + ", pileci skok:" + jumpSafetyCheck + ", check down:" + triggerCheckDown + ", v: " + GetComponent<Rigidbody2D>().velocity.y + ", in air: " + inAir + ", tforce: " + tempForce + ", speed: " + GetComponent<Rigidbody2D>().velocity.x);
        ...
        else if (state == State.jumped) // ************* Player is in the air
        {
            ...
            {
                GetComponent<Rigidbody2D>().velocity =
                    new Vector2(maxSpeedX, GetComponent<Rigidbody2D>().velocity.y + korakce);
                if (korakce > 0)
                    korakce -= 0.085f;
                //korakce -= Time.deltaTime * 5.5f;
            }
            ...
            else if (jumpControlled)
            {
                ...
                {
                    //GetComponent<Rigidbody2D>().velocity = Vector2.zero;
                    //GetComponent<Rigidbody2D>().AddForce(new Vector2(0, tempForce));
                    //GetComponent<Rigidbody2D>().velocity = new Vector2(GetComponent<Rigidbody2D>().velocity.x,startVelY + maxSpeedY);
                }
            ...
            {
                //GetComponent<Rigidbody2D>().velocity = new Vector2(Mathf.MoveTowards(jumpSpeedX,jumpSpeedX*1.75f,0.35f), GetComponent<Rigidbody2D>().velocity.y);

                //majmun.GetComponent<Animation>().PlayQueued(fallingAnimation.name,QueueMode.CompleteOthers); //ZBOG ANIMATORA!!!!
                //jumpSafetyCheck = false;

                //GetComponent<Rigidbody2D>().gravityScale = 11.5f;
                //jumpSpeedX = 13;
        ...
        else if (state == State.climbUp) // ************* Player is climbing
        {
            ...
            else if (jumpControlled)
            {
                ...
                if (Mathf.Abs(GetComponent<Rigidbody2D>().velocity.y) > maxSpeedY)
                    GetComponent<Rigidbody2D>().velocity =
                        new Vector2(GetComponent<Rigidbody2D>().velocity.x, maxSpeedY);
            }
        ...
        // -------------- Provera za pileci skok i tobogan --------------
        // --------------------------------------------------------------
        if (!triggerCheckDown || Physics2D.Linecast(transform.position + new Vector3(1f, 0, 0),
                transform.position + new Vector3(1f, 2f, 0), 1 << LayerMask.NameToLayer("Ground")))
        {
            if (jumpSafetyCheck)
                jumpSafetyCheck = false;
        }
    ...
    void OnTriggerExit2D(Collider2D col)
    {
        if (col.tag == "Footer")
        {
            ...
            //Debug.Log("exit");
            if (GetComponent<Collider2D>().isTrigger && (groundCheck.position.y + 0.2f > triggerPositionUp ||
                                                         ceilingCheck.position.y <
                                                         triggerPositionDown)) // && !triggerCheckDownTrigger)
            {
        ...
        if (state != State.completed || state != State.wasted)
        {
            ...
            else if (col.gameObject.tag == "Footer")
            {
                ...
                if (state == State.saZidaNaZid)
                {
                    ...
                    majmun.localScale = new Vector3(majmun.localScale.x, majmun.localScale.y,
                        Mathf.Abs(majmun.localScale.z));
                    if (klizanje.isPlaying)
                        klizanje.Stop();
                ...
                if (state == State.jumped)
                {
                    ...
                    if (proveraGround < 12)
                    {
                        ...
                        if (powerfullImpact)
                        {
                            ...
                            //majmun.Find("PorinceGorilla_LP").GetComponent<Renderer>().material.color = new Color(0.53f,0.53f,0.53f,1);
            ...
            else if (col.gameObject.tag == "Enemy")
            {
                ...
                if (activeShield)
                {
                    ...
                    if (state != State.running)
                        ...
                        GetComponent<Rigidbody2D>().AddForce(new Vector2(maxSpeedX, 1100));
                }
                else if (!killed)
                {
                    if (state == State.running)
    ...
    IEnumerator ProceduraPenjanja(GameObject obj)
    {
        ...
        GetComponent<Rigidbody2D>().isKinematic = false;
        //majmun.GetComponent<Animation>().Play(runAnimation.name); //ZBOG ANIMATORA!!!!
        mozeDaSkociOpet = false;
        animator.SetBool("Jump", false);
        animator.SetBool("DoubleJump", false);
        animator.SetBool("Glide", false);
    ...
    IEnumerator snappingProcess()
    {
        float t = 0;
        float step = 0.25f;
        while (t < 0.02f && (Mathf.Abs(transform.position.x - colliderForClimb.x) > 0.01f &&
                             Mathf.Abs(transform.position.y - colliderForClimb.y) > 0.01f)) //0.03f)
        {
            ...
            transform.position = Vector3.Lerp(transform.position,
                new Vector3(colliderForClimb.x, colliderForClimb.y, transform.position.z), step);
        ...
        if (col.gameObject.tag == "Footer")
        {
            if (state == State.running)
            {
                ...
                if (!proveriTerenIspredY && !downHit) // ************* 2.5m ispred nema nicega ispod
                {
                    ...
                    if (runParticle.isPlaying)
                        runParticle.Stop();
                    //majmun.GetComponent<Animation>().Play(fallingAnimation.name); //ZBOG ANIMATORA!!!!
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        if (state != State.completed || state != State.wasted)
        {
            ...
            else if (col.name == "Banana_collect")
            {
                col.gameObject.SetActive(false);
                Manage.points += 200;
                //Manage.pointsText.text = Manage.pointsText_Senka.text = Manage.points.ToString();
                StartCoroutine(reappearItem(col.gameObject));
            }
            ...
            else if (col.tag == "Footer")
            {
                ...
                //Debug.Log("enter");
                if ((transform.position.y + 0.25f > triggerPosition) && triggerCheckDownTrigger &&
                    triggerCheckDownBehind && GetComponent<Collider2D>().isTrigger)
                {
                    GetComponent<Collider2D>().isTrigger = false;
    ...
    void OnTriggerStay2D(Collider2D col)
    {
        if (col.tag == "GrabLedge")
            GetComponent<Rigidbody2D>().velocity =
                new Vector2(0, Mathf.MoveTowards(GetComponent<Rigidbody2D>().velocity.y, 0, 0.2f));
    }
    ...
    IEnumerator pratiLijanaTarget(Transform target)
    {
        while (lijana)
        {
            yield return null;
            transform.position = Vector3.Lerp(transform.position,
                new Vector3(target.position.x, target.position.y, transform.position.z), 0.2f);
    ...
    IEnumerator ClimbLedge(Transform target, float time)
    {
        ...
        //while(majmun.GetComponent<Animation>().IsPlaying(climbAnimation.name))
        yield return null;
        //stop = false;
    ...
    IEnumerator MoveUp(float time)
    {
        ...
        {
            transform.position =
                Vector2.MoveTowards(transform.position, new Vector3(destX, target, transform.position.z), t);
            if (Time.timeScale != 0)
                offsetX = 2f;
            else
                offsetX = 0;
    ...
    public void majmunUtepan()
    {
        ...
        //majmun.GetComponent<Animation>().Stop();
        //majmun.GetComponent<Animation>().Play(deathAnimation.name); //ZBOG ANIMATORA!!!!
    ...
    IEnumerator slowDown()
    {
        ...
        {
            transform.position = Vector3.Lerp(transform.position,
                new Vector3(finish, transform.position.y, transform.position.z), t);
            t += Time.deltaTime / 2;
            yield return null;
        }
