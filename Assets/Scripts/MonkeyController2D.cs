...
//		}
        ...
        if (state == State.jumped || state == State.climbUp)
        {
            ...
            if ((Input.GetMouseButton(0))) // && Input.mousePosition.x < Screen.width/2)
            {
                ...
                if (SlideNaDole)
                {
                    if ((startY - endY) > ((1f / 8f * Camera.main.orthographicSize) *
        ...
        if (state == State.wallhit)
        {
            if (KontrolisaniSkok)
            {
                    ...
                    Input.GetKeyDown(KeyCode.Space))
                {
                    ...
                    if (RaycastFunction(Input.mousePosition) != "Pause Button" &&
                        RaycastFunction(Input.mousePosition) != "Play Button_Pause" &&
                        RaycastFunction(Input.mousePosition) != "Buy Button" &&
                        RaycastFunction(Input.mousePosition) != "ButtonMain_NoMoreLives" &&
                        RaycastFunction(Input.mousePosition) != "Restart Button_Pause" &&
                        !RaycastFunction(Input.mousePosition).Contains("Tutorial") &&
                        !RaycastFunction(Input.mousePosition).Contains("Power_") &&
                        RaycastFunction(Input.mousePosition) != "Menu Button_Pause") // ZA PRVU VERZIJU
                    {
                        duzinaPritiskaZaSkok = Time.time;
...
//			}
        ...
        if (state == State.lijana)
        {
            ...
            if (Input.GetMouseButtonDown(0) && Input.mousePosition.x > povrsinaZaClick)
            {
                if (heCanJump && RaycastFunction(Input.mousePosition) != "Pause Button" &&
                    RaycastFunction(Input.mousePosition) != "Play Button_Pause" &&
                    RaycastFunction(Input.mousePosition) != "Buy Button" &&
                    RaycastFunction(Input.mousePosition) != "ButtonMain_NoMoreLives" &&
                    RaycastFunction(Input.mousePosition) != "Restart Button_Pause" &&
                    !RaycastFunction(Input.mousePosition).Contains("Tutorial") &&
                    !RaycastFunction(Input.mousePosition).Contains("Power_") &&
                    RaycastFunction(Input.mousePosition) != "Menu Button_Pause")
                {
...
//				GetComponent<Rigidbody2D>().velocity = new Vector2(maxSpeedX, GetComponent<Rigidbody2D>().velocity.y);
        ...
        else if (state == State.jumped) // ************* Player is in the air
        {
            ...
            if (swoosh)
            {
                ...
                //GetComponent<Rigidbody2D>().velocity = new Vector2(maxSpeedX,-100);
                swoosh = false;
            }
            ...
            else if (col.gameObject.tag == "Enemy")
            {
                if (activeShield)
                {
                    ...
                    if (state != State.running)
                    {
                        ...
                        //GetComponent<Rigidbody2D>().AddForce(new Vector2(maxSpeedX,0));
    ...
    IEnumerator FallDownAfterSpikes()
    {
        ...
        state = State.wasted;
        manage.transform.Find("Gameplay Scena Interface/_TopRight/Pause Button").GetComponent<Collider>().enabled =
            false;
    ...
    void OnCollisionExit2D(Collision2D col)
    {
        if (col.gameObject.tag == "Footer")
        {
            if (state == State.running)
            {
                ...
                if (!Physics2D.Linecast(transform.position + new Vector3(4.4f, 1.2f, 0),
                        transform.position + new Vector3(4.4f, -3.2f, 0),
                        1 << LayerMask.NameToLayer("Ground") | 1 << LayerMask.NameToLayer("Platform")) &&
                    !Physics2D.Linecast(transform.position + new Vector3(0.2f, 0.1f, 0),
                        ...
                        1 << LayerMask.NameToLayer("Ground") | 1 << LayerMask.NameToLayer("Platform")))
                {
                    ...
                    if (!Physics2D.Linecast(transform.position + new Vector3(2.3f, 1.25f, 0),
...
//			}
            ...
            {
                col.transform.Find("+3CoinsHolder/+3Coins").GetComponent<TextMesh>().text = col.transform.transform
                    .Find("+3CoinsHolder/+3Coins/+3CoinsShadow").GetComponent<TextMesh>().text = "+500";
            ...
            {
                col.transform.Find("+3CoinsHolder/+3Coins").GetComponent<TextMesh>().text = col.transform.transform
                    .Find("+3CoinsHolder/+3Coins/+3CoinsShadow").GetComponent<TextMesh>().text = "+250";
                col.transform.Find("+3CoinsHolder").GetComponent<Animator>().Play("FadeOutCoins");
                col.transform.Find("AnimationHolder").GetComponent<Animation>().Play("CoinsBagCollect");
            ...
            {
                col.transform.Find("+3CoinsHolder/+3Coins").GetComponent<TextMesh>().text = col.transform.transform
                    .Find("+3CoinsHolder/+3Coins/+3CoinsShadow").GetComponent<TextMesh>().text = "+100";
                col.transform.Find("+3CoinsHolder").GetComponent<Animator>().Play("FadeOutCoins");
                col.transform.Find("AnimationHolder").GetComponent<Animation>().Play("CoinsBagCollect");
...
//				StartCoroutine("pratiLijanaTarget",grabLianaTransform);
    ...
    IEnumerator ReduceMaxSpeedGradually()
    {
        ...
        //GetComponent<Rigidbody2D>().mass = 350;
...
//		}
        while (true)
        {
            ...
            transform.position = Vector3.Lerp(transform.position,
                new Vector3(target.position.x, target.position.y, transform.position.z), 0.25f);
        }
    }
...
//		animator.Play(jump_State);
    ...
    public void SetInvincible()
    {
        ...
        state = State.jumped;
        GetComponent<Rigidbody2D>().isKinematic = false;
        //GetComponent<Rigidbody2D>().AddForce(new Vector2(1500,1000));
    ...
    IEnumerator blink()
    {
        ...
        if (StagesParser.glava != -1)
        {
            glava = majmun.Find("ROOT/Hip/Spine/Chest/Neck/Head/" + StagesParser.glava).GetChild(0)
    ...
    IEnumerator pojaviMaglu(Collider2D col)
    {
        ...
        while (t < 1)
        {
            fogOfWar.localScale = new Vector3(Mathf.Lerp(fogOfWar.localScale.x, target, t), fogOfWar.localScale.y,
                fogOfWar.localScale.z);
        ...
        while (t < 1)
        {
            fogOfWar.localScale = new Vector3(Mathf.Lerp(fogOfWar.localScale.x, value, t), fogOfWar.localScale.y,
                fogOfWar.localScale.z);
            //fogOfWar.localScale = Vector3.Lerp(fogOfWar.localScale, new Vector3(target,fogOfWar.localScale.y,fogOfWar.localScale.z),t);
            t += Time.deltaTime / 1.2f;
            yield return null;
    ...
    IEnumerator Spusti2DFollow()
    {
        ...
        {
            yield return null;
            cameraTarget.transform.localPosition = new Vector3(cameraTarget.transform.localPosition.x,
                Mathf.MoveTowards(cameraTarget.transform.localPosition.y, target.y, 0.2f),
    ...
    IEnumerator Podigni2DFollow()
    {
        while (cameraTarget.transform.localPosition.y <= originalCameraTargetPosition - 0.1f)
        {
            yield return null;
            cameraTarget.transform.localPosition = new Vector3(cameraTarget.transform.localPosition.x,
                Mathf.MoveTowards(cameraTarget.transform.localPosition.y, originalCameraTargetPosition, 0.1f),
                cameraTarget.transform.localPosition.z);
        }

        cameraTarget.transform.localPosition = new Vector3(cameraTarget.transform.localPosition.x,
            originalCameraTargetPosition, cameraTarget.transform.localPosition.z);
    }
