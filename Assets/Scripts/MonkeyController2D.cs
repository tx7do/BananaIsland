...
//		}
		...
		if(StagesParser.majica != -1)
		{
			...
			majmun.Find("custom_Majica").renderer.material.SetTexture("_MainTex", MajicaTekstura);
			majmun.Find("custom_Majica").renderer.material.color = StagesParser.bojaMajice;
		}
			...
			if(KontrolisaniSkok)
			{
				...
				if((Input.GetMouseButtonDown(0) && Input.mousePosition.x > povrsinaZaClick) || Input.GetKeyDown(KeyCode.Space))
				{
					...
					{	
						...
						proveraGround = 16;
						korakce = 3;
						rigidbody2D.velocity = new Vector2(maxSpeedX,maxSpeedY);
		...
		if(state == State.saZidaNaZid) 		// ************* Player is bouncing from wall to wall
		{
			if(jump)
			{
				...
				if(rigidbody2D.velocity.y < maxSpeedY)
				{
					...
					rigidbody2D.AddForce(new Vector2(maxSpeedX, 2500));
				}
...
//				rigidbody2D.velocity = new Vector2(maxSpeedX, rigidbody2D.velocity.y);
		...
		else if(state == State.jumped) 	// ************* Player is in the air
		{
			if(jumpHolding)
			{
				rigidbody2D.velocity = new Vector2(maxSpeedX, rigidbody2D.velocity.y + korakce);
				if(korakce > 0)
					korakce -= 0.085f;
			...
			if(swoosh)
			{
				...
				rigidbody2D.velocity = new Vector2(maxSpeedX,0);
				rigidbody2D.AddForce(new Vector2(maxSpeedX,-4500));
			...
			if(doubleJump)
			{
				...
				rigidbody2D.velocity = Vector2.zero;
				rigidbody2D.AddForce(new Vector2(2000, doubleJumpForce));
				doubleJump = false;
				hasJumped = false;
		...
		else if(state == State.climbUp)	// ************* Player is climbing
		{
			...
			else if(jump)
			{
				...
				rigidbody2D.AddForce(new Vector2(maxSpeedX, jumpForce));
				hasJumped = true;
				jump = false;
			}
			else if(jumpControlled)
			{
				...
				if(rigidbody2D.velocity.y < maxSpeedY)
				{
					rigidbody2D.AddForce(new Vector2(maxSpeedX, tempForce));
				}
	...
	void OnTriggerExit2D(Collider2D col)
	{
		if(col.tag == "Footer")
		{
			...
			//Debug.Log("exit");
			if(collider2D.isTrigger && (groundCheck.position.y +0.2f > triggerPositionUp || ceilingCheck.position.y < triggerPositionDown))// && !triggerCheckDownTrigger)
			{
		...
		else if(state != State.completed || state != State.wasted)
		{
			...
			else if(col.gameObject.tag == "Enemy")
			{
				if(activeShield)
				{
					...
					enemyCollider = col.transform.collider2D;
					enemyCollider.enabled = false;
					Invoke("EnableColliderBackOnEnemy",1f);
					activeShield = false;
					transform.Find("Particles/ShieldDestroyParticle").particleSystem.Play();
					transform.Find("Particles/ShieldDestroyParticle").GetChild(0).particleSystem.Play();
				...
				else if(!killed && !invincible)
				{
					...
					transform.Find("Particles/OblakKill").particleSystem.Play();
	...
	IEnumerator FallDownAfterSpikes()
	{
		...
		state = State.wasted;
		manage.transform.Find("Gameplay Scena Interface/_TopRight/Pause Button").collider.enabled = false;
		...
		//rigidbody2D.isKinematic = true;
		rigidbody2D.velocity = Vector2.zero;
		rigidbody2D.velocity = new Vector2(-8,30);
	...
	IEnumerator AfterFallDown()
	{
		...
		rigidbody2D.velocity = new Vector2(-4,22);
...
//			}
			...
			{
				col.transform.GetChild(0).renderer.enabled = false;
				Camera.main.transform.Find("FogOfWar").renderer.enabled = true;
			...
			else if(col.name.Contains("CoinsBagBig"))
			{
				...
				col.transform.Find("AnimationHolder").animation.Play("CoinsBagCollect");
...
//			}
			...
			else if(col.tag == "Footer" && col.gameObject.layer == LayerMask.NameToLayer("Platform"))
			{
				...
				//Debug.Log("enter");
				if((transform.position.y + 0.25f > triggerPosition) && triggerCheckDownTrigger && triggerCheckDownBehind && collider2D.isTrigger)
				{
					collider2D.isTrigger = false;
...
//				StartCoroutine("pratiLijanaTarget",grabLianaTransform);
	...
	IEnumerator ReduceMaxSpeedGradually()
	{
		...
		maxSpeedX = 0;
		dancing = true;
		collider2D.sharedMaterial = finishDontMove;
...
//		animator.Play(jump_State);
		if(state == State.lijana)
		{
			...
			rigidbody2D.isKinematic = false;
			rigidbody2D.velocity = Vector2.zero;
			rigidbody2D.AddForce(new Vector2(1000,2500));
...
//		animator.Play(jump_State);
	...
	public void SetInvincible()
	{
		...
		rigidbody2D.velocity = new Vector2(maxSpeedX,50);
	...
	IEnumerator blink()
	{
		...
		int i = 0;
		bool radi = false;
		Renderer meshrenderer = majmun.Find("PorinceGorilla_LP").renderer;
		Renderer usi = majmun.Find("Usi").renderer;
		Renderer kosa = majmun.Find("Kosa").renderer;
		Renderer glava = null;
		...
		if(StagesParser.glava != -1)
		{
			glava = majmun.Find("ROOT/Hip/Spine/Chest/Neck/Head/"+StagesParser.glava).GetChild(0).renderer;
		}
		if(StagesParser.majica != -1)
		{
			majica = majmun.Find("custom_Majica").renderer;
		}
		if(StagesParser.ledja != -1)
		{
			ledja = majmun.Find("ROOT/Hip/Spine/"+StagesParser.ledja).GetChild(0).renderer;
		}
	...
	IEnumerator RemoveFog(float time, Collider2D col)
	{
		yield return new WaitForSeconds(time);
		col.enabled = true;
		col.transform.GetChild(0).renderer.enabled = true;
