...
//		}
    ...
    void Update()
    {
        hit = Physics2D.Linecast(transform.position + new Vector3(0.8f, 1f, 0),
            transform.position + new Vector3(0.8f, -35.5f, 0),
        ...
        //if(canDo)
        {
            ...
            if (((transform.position.x < Camera.main.ViewportToWorldPoint(Vector3.one).x + distance) ||
                 animateInstantly) && proveraJednom)
            {
                proveraJednom = false;
...
////		}
    ...
    void FixedUpdate()
    {
        if (patrolinjo)
        {
            ...
            parentRigidbody2D.velocity = /*pravac.x*5*parentRigidbody2D.velocity.normalized;*/
                new Vector2(pravac.x * 5, parentRigidbody2D.velocity.y);
        ...
        else if ((run || runAndJump) && runinjo)
        {
            ...
            //if(Mathf.Abs(GetComponent<Rigidbody2D>().velocity.x) > maxSpeedX)
            {
                ...
                if (runAndJump)
                {
                    ...
                    {
                        baboonShadow.position = new Vector3(baboonShadow.position.x, hit.point.y + 0.3f,
                            baboonShadow.position.z);
                    }
                }
...
//			bullet.GetComponent<Rigidbody2D>().AddForce(new Vector2(-2300,0));
//			canShoot = false;
//		}
//		if(jumpNow)
//		{
//			Debug.Log("A KOJO MOOO");
//			//GetComponent<Rigidbody2D>().velocity = new Vector2(GetComponent<Rigidbody2D>().velocity.x, 200);
    ...
    IEnumerator destroyBabun()
    {
        ...
        else
        {
            ...
            else if (boomerang)
            {
                ...
                kolicinaPoena = 70;
                //Boomerang.GetComponent<Collider2D>().enabled = false;
        ...
        ThreeCoinsHolder.Find("+3Coins").GetComponent<TextMesh>().text =
            ThreeCoinsHolder.Find("+3Coins/+3CoinsShadow").GetComponent<TextMesh>().text = "+" + value;
...
//		}
    ...
    public void killBaboonStuff()
    {
        ...
        if (!impact)
        {
            ...
            //player.GetComponent<Rigidbody2D>().AddForce(new Vector2(0, player.jumpForce - GetComponent<Rigidbody2D>().velocity.y*player.doubleJumpForce));
...
//		if(player.transform.position.y + 1.25f < reqHeight.position.y && player.activeShield)
    ...
    public void ResetujBabuna()
    {
        ...
        //Boomerang.GetComponent<Collider2D>().enabled = true;
        if (koplje)
