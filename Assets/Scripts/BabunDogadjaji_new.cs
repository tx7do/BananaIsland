...
public class BabunDogadjaji_new : MonoBehaviour {
	...
	void Awake()
	{
		...
		parentRigidbody2D = transform.parent.rigidbody2D;
		//canDo = false;
		//this.enabled = false;
	...
	IEnumerator destroyBabun()
	{
		if(isGorilla)
		{
			...
			else if(koplje)
			{
				...
				kolicinaPoena = 70;
				Koplje.collider2D.enabled = false;
			}
			else if(jump)
			{
				kolicinaPoena = 50;
...
//		}
	...
	IEnumerator bounceOffEnemy()
	{
		...
		if(!player.isSliding)
		{
			player.rigidbody2D.velocity = new Vector2(player.maxSpeedX,0);
			player.rigidbody2D.AddForce(new Vector2(0, 1500));
			player.animator.Play(player.jump_State);
		}
		else
		{
			...
			{
				player.rigidbody2D.velocity = new Vector2(player.maxSpeedX,0);
				player.rigidbody2D.AddForce(new Vector2(0, -2500));
...
//		if(player.transform.position.y + 1.25f < reqHeight.position.y && player.activeShield)
	//	Debug.Log("Stiti: " + player.activeShield);
		if(player.activeShield || player.invincible || player.powerfullImpact)
		{
			...
			if(player.activeShield)
			{
				...
				player.transform.Find("Particles/ShieldDestroyParticle").particleSystem.Play();
				player.transform.Find("Particles/ShieldDestroyParticle").GetChild(0).particleSystem.Play();
	...
	void PogasiBabuna()
	{
		...
		transform.parent.Find("Baboon").renderer.enabled = false;
		anim.enabled = false;
		patrolinjo = false;
		flyinjo = false;
		runinjo = false;
	...
	public void ResetujBabuna()
	{
		...
		//colliders[2].enabled = true;
		reqHeight.GetComponent<KillTheBaboon>().turnOnColliders();
		transform.parent.Find("Baboon").renderer.enabled = true;
		anim.enabled = true;
		this.enabled = true;
		ugasen = false;
		proveraJednom = true;
		...
		if(koplje)
			Koplje.collider2D.enabled = true;
