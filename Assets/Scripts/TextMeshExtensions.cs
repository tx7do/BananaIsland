...
//	}
    ...
    public static void AdjustFontSize(this TextMesh toAdjust, bool rowSplit, bool hasWhiteSpaces,
        bool increaseFont /* = true*/)
    {
        //mora da postoji collider na osnovu koga proverava velicinu
        //zastita ako nije postavljen
        if (toAdjust.GetComponent<Collider>() != null)
        {
            if (toAdjust.GetComponent<Collider>().bounds.size.x == 0 ||
                toAdjust.GetComponent<Collider>().bounds.size.y == 0) return;
            string oldText = toAdjust.text;
            //provera ako treba da se poveca tekst
            if (increaseFont)
            {
                while (((toAdjust.GetComponent<Renderer>().bounds.size.x * 1.1f) <
                        toAdjust.GetComponent<Collider>().bounds.size.x) &&
                       ((toAdjust.GetComponent<Renderer>().bounds.size.y * 1.1f) <
                        toAdjust.GetComponent<Collider>().bounds.size.y))
                {
                    toAdjust.characterSize *= 1.1f;
                }
            }
            ...
            {
                if (((toAdjust.GetComponent<Renderer>().bounds.size.x) <
                     toAdjust.GetComponent<Collider>().bounds.size.x) &&
                    ((toAdjust.GetComponent<Renderer>().bounds.size.y) <
                     toAdjust.GetComponent<Collider>().bounds.size.y))
                {
                    return;
                }
            ...
            if (!rowSplit)
            {
                while (((toAdjust.GetComponent<Renderer>().bounds.size.x) >
                        toAdjust.GetComponent<Collider>().bounds.size.x) ||
                       ((toAdjust.GetComponent<Renderer>().bounds.size.y) >
                        toAdjust.GetComponent<Collider>().bounds.size.y))
                {
                ...
                else
                {
                    ...
                    while (novCount < wholeText.Length)
                    {
                        ...
                        else
                        {
                            ...
                            toAdjust.text +=
                                ((hasWhiteSpaces) ? " " : "") + wholeText[novCount++]; //Debug.Log("s: "+toAdjust.text);
                            if (((toAdjust.GetComponent<Renderer>().bounds.size.y >
                                  toAdjust.GetComponent<Collider>().bounds.size.y) ||
                                 (toAdjust.GetComponent<Renderer>().bounds.size.x >
                                  toAdjust.GetComponent<Collider>().bounds.size.x)))
                            {
                                toAdjust.text = back + "\n" + wholeText[novCount - 1];
                                ; //Debug.Log("s: "+toAdjust.text);
                            }
                        }

                        while (((toAdjust.GetComponent<Renderer>().bounds.size.y >
                                 toAdjust.GetComponent<Collider>().bounds.size.y) ||
                                (toAdjust.GetComponent<Renderer>().bounds.size.x >
                                 toAdjust.GetComponent<Collider>().bounds.size.x)))
                        {
        ...
        if (outLine)
        {
            if (toAdjust != null)
            {
                if (toAdjust.transform.childCount == 8)
                {
                    if (otherChildren)
                    {
                        toAdjust.transform.Find(toAdjust.name + "RightOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                        toAdjust.transform.Find(toAdjust.name + "LeftOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                        toAdjust.transform.Find(toAdjust.name + "UpOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                        toAdjust.transform.Find(toAdjust.name + "DownOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                        toAdjust.transform.Find(toAdjust.name + "LeftUpOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                        toAdjust.transform.Find(toAdjust.name + "RightUpOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                        toAdjust.transform.Find(toAdjust.name + "LeftDownOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                        toAdjust.transform.Find(toAdjust.name + "RightDownOutline").GetComponent<TextMesh>().text =
                            toAdjust.text;
                    }
                    else
                    {
