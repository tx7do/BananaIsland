...
//	}
	...
	public static void AdjustFontSize(this TextMesh toAdjust,bool rowSplit, bool hasWhiteSpaces, bool increaseFont/* = true*/)
	{
		...
		if(toAdjust.collider!=null)
		{
			if(toAdjust.collider.bounds.size.x==0||toAdjust.collider.bounds.size.y==0) return;
			string oldText=toAdjust.text;
			//provera ako treba da se poveca tekst
			if(increaseFont)
			{
			...
			{
				if(((toAdjust.renderer.bounds.size.x )<toAdjust.collider.bounds.size.x)&&((toAdjust.renderer.bounds.size.y )<toAdjust.collider.bounds.size.y))
				{
					return;
				}
			}
				...
				else
				{
					...
					{ 
						...
						else
						{
							...
							if(((toAdjust.renderer.bounds.size.y>toAdjust.collider.bounds.size.y)||(toAdjust.renderer.bounds.size.x>toAdjust.collider.bounds.size.x)))
							{
								toAdjust.text=back+"\n"+wholeText[novCount-1];;//Debug.Log("s: "+toAdjust.text);
							}
						}
						while(((toAdjust.renderer.bounds.size.y>toAdjust.collider.bounds.size.y)||(toAdjust.renderer.bounds.size.x>toAdjust.collider.bounds.size.x)))
						{
							string back= toAdjust.text;
...
//			Debug.LogError(toAdjust.gameObject.name+"##"+toAdjust.gameObject.GetComponents<MonoBehaviour>().Length);
			...
			GameObject help1= GameObject.Instantiate(toAdjust.gameObject) as GameObject;
			if(help1.collider != null)
				Object.Destroy(help1.collider);
			...
			GameObject help2= GameObject.Instantiate(toAdjust.gameObject) as GameObject;
			if(help2.collider != null)
				Object.Destroy(help2.collider);
			...
			GameObject help3= GameObject.Instantiate(toAdjust.gameObject) as GameObject;
			if(help3.collider != null)
				Object.Destroy(help3.collider);
			...
			GameObject help4= GameObject.Instantiate(toAdjust.gameObject) as GameObject;
			if(help4.collider != null)
				Object.Destroy(help4.collider);
			...
			if(help5.collider != null)
				Object.Destroy(help5.collider);
			...
			if(help6.collider != null)
				Object.Destroy(help6.collider);
			...
			if(help7.collider != null)
				Object.Destroy(help7.collider);
			...
			if(help8.collider != null)
				Object.Destroy(help8.collider);
