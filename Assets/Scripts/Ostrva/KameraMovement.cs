	...
	void Awake()
	{
		...
		else //bonus nivo
		{
			...
			if(!StagesParser.dodatnaProveraIzasaoIzBonusa)
			{
				...
				kovcegNaPocetku.collider.enabled = false;
...
//			}
			...
			{//locked
				...
				GameObject.Find("LevelsWorld" + StagesParser.currSetIndex + "/Level" + (i+1) + "/Level" + (i+1) + "Move").transform.Find("Katanac").renderer.enabled = true;
			...
			{//unlocked
				...
				if((i+1) != StagesParser.zadnjiOtkljucanNivo || StagesParser.pozicijaMajmuncetaNaMapi != Vector3.zero)
				{
					...
					GameObject.Find("LevelsWorld" + StagesParser.currSetIndex + "/Level" + (i+1) + "/Level" + (i+1) + "Move").transform.Find("Katanac").renderer.enabled = false;
		...
		if(Input.GetMouseButtonUp(0))
		{
			...
			if(clickedItem == releasedItem && (Time.time - trajanjeKlika < 0.35f) && (Mathf.Abs(Input.mousePosition.x - pomerajOdKlika_X) < 80 && Mathf.Abs(Input.mousePosition.y - pomerajOdKlika_Y) < 80))
			{
				...
				else if(releasedItem.StartsWith("Kovceg"))
				{
					...
					{
						if(pronadjiKovceg(StagesParser.bonusName).Find("GateClosed").renderer.enabled)
						{
...
//							"OVO DA SE PREMESTI DOLE!!!!"
							...
							trenutniKovceg.Find("GateClosed").renderer.enabled = false;
							trenutniKovceg.Find("GateOpen").renderer.enabled = true;
							animator.Play("Running");
							StartCoroutine("KretanjeMajmunceta");
						}
					}
				...
				else if(releasedItem.Equals("Button_UnlockBonusYES"))
				{
					...
					if(int.Parse(values[2]) < zadnjiOtkljucanNivo_proveraZaBonus)
					{
						...
						{
							_GUI.Find("INTERFACE HOLDER/_TopLeft/BananaHolder/Bananas").animation.Play();
						}
					}
					else
					{
				...
				else if(releasedItem.Equals("ShopKucica"))
				{
					...
					_GUI.Find("ShopHolder/Shop").animation.Play("MeniDolazak");
...
//			}
	...
	void ocistiMisije()
	{
		...
		for(int i=0;i<mission1Icons.childCount;i++)
			mission1Icons.GetChild(i).renderer.enabled = false;
		for(int i=0;i<mission2Icons.childCount;i++)
			mission2Icons.GetChild(i).renderer.enabled = false;
		for(int i=0;i<mission3Icons.childCount;i++)
			mission3Icons.GetChild(i).renderer.enabled = false;
...
//		while ( animation.IsPlaying(animName) );
	...
	IEnumerator PokaziMuCustomize()
	{
		...
		TutorialShop.transform.GetChild(0).animation.Play();
		yield return new WaitForSeconds(0.5f);
		TutorialShop.transform.GetChild(0).Find("RedArrowHolder/RedArrow").renderer.enabled = true;
		TutorialShop.transform.GetChild(0).Find("RedArrowHolder/RedArrow").animation.Play();
		TutorialShop.transform.GetChild(0).collider.enabled = false;
		...
		for(int i=0;i<scoresAndIndexes.Count;i++) //scoresAndIndexes.Count
		{
			...
			if(i<5)
			{
				if(scoresAndIndexes[i].score > 0 || FacebookManager.ListaStructPrijatelja[scoresAndIndexes[i].index].PrijateljID==FacebookManager.User)
				{
					...
					trenutniPrijatelj.Find("Friends Level Win " + redosled+"/Friends Level Win Picture " + redosled).renderer.material.mainTexture = FacebookManager.ListaStructPrijatelja[scoresAndIndexes[i].index].profilePicture;
