...
//				manage.AddPoints(6);
    ...
    IEnumerator magnetWorking()
    {
        ...
        while (t < 0.25f)
        {
            if (((transform.position.x < Monkey.position.x + 3f &&
                  transform.position.x >=
                  Monkey.position.x -
                  2f) /*&& (transform.position.y < Monkey.position.y + 1.25f && transform.position.y > Monkey.position.y - 1.25f)*/
                ) && !magnetAnimacija)
            {
            ...
            transform.position = Vector3.Lerp(transform.position,
                Monkey.position + new Vector3(1, 1, 0) /* + Vector3.right*/, t);
            t += Time.deltaTime / 3;
            yield return null;
        }
