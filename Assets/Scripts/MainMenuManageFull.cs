...
//		majmunLogo = GameObject.Find("HolderMajmun");
		...
		Languages.animation.Play("MainLanguageFlagsPosition");
	...
	void Update () 
	{
		...
		if(Input.GetMouseButtonUp(0))
		{
			...
			else if(RaycastFunction(Input.mousePosition) == "MainLeaderboardArrow")
			{
				...
				{
					LeaderBoard.animation.Play("MainLeaderboardGo");
				}
				else
				{
					LeaderBoard.animation.Play("MainLeaderboardShow");
				}
			...
			else if(RaycastFunction(Input.mousePosition) == "ButtonLanguage")
			{
				...
				if(LanguagesAktivan)
				{
					...
					Languages.animation.Play("MainLanguageFlagsShow");
